...
extern "C" {
    ...
    struct llama_cacheflow_cuda_stats {
        ...
        uint64_t remap_vectorized_bytes;
        uint64_t remap_scalar_bytes;
    };
...
#define LLAMA_STATE_SEQ_FLAGS_ON_DEVICE 2
    ...
    enum llama_kv_execution_action {
        LLAMA_KV_EXECUTION_ACTION_DIRECT = 0,
        LLAMA_KV_EXECUTION_ACTION_REMAP  = 1,
        LLAMA_KV_EXECUTION_ACTION_PAGED  = 2,
    };

    // Stable, public reason codes for a requested Paged action becoming
    // Direct before any KV mutation.
    enum llama_kv_execution_fallback_reason {
        LLAMA_KV_EXECUTION_FALLBACK_NONE                 = 0,
        LLAMA_KV_EXECUTION_FALLBACK_INVALID_PAGE_SIZE    = 1,
        LLAMA_KV_EXECUTION_FALLBACK_EMPTY_SEQUENCE       = 2,
        LLAMA_KV_EXECUTION_FALLBACK_MISSING_POSITION     = 3,
        LLAMA_KV_EXECUTION_FALLBACK_DUPLICATE_POSITION   = 4,
        LLAMA_KV_EXECUTION_FALLBACK_NON_CONTIGUOUS_BLOCK = 5,
        LLAMA_KV_EXECUTION_FALLBACK_UNSUPPORTED_REQUEST  = 6,
        LLAMA_KV_EXECUTION_FALLBACK_UNSUPPORTED_MODEL    = 7,
    };

    struct llama_kv_execution_status {
        enum llama_kv_execution_action requested;
        enum llama_kv_execution_action effective;
        enum llama_kv_execution_fallback_reason fallback_reason;
        uint64_t paged_decode_calls;
        uint64_t paged_decode_fallbacks;
        uint64_t paged_decode_sequences;
        uint32_t paged_decode_max_batch;
        uint64_t paged_decode_contiguous_fastpath_calls;
        uint64_t paged_decode_contiguous_fastpath_sequences;
    };

    LLAMA_API void llama_set_kv_execution_action(
            struct llama_context * ctx,
            enum llama_kv_execution_action action);

    LLAMA_API struct llama_kv_execution_status llama_get_kv_execution_status(
