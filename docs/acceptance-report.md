...
| #5 | 已关闭 | 受限 Qwen Paged Decode K1–K4 CUDA kernel、FP32 stable softmax、GQA7 整组 KV reuse、CPU oracle、跨页/非连续页和 fail-closed 测试；K4 长上下文性能仍未晋级。 |
...
K2 针对生产 `14Q/2KV/D64` 的 GQA7 envelope，把 K1 的每 query-head CTA 改为每 CTA 两个 query-head warp；每个 CTA 共享装载其两个 query heads 的 K/V，使每 KV head 的装载 tile 从 7 个降为 4 个。K 采用按维度转置的 shared-memory 布局，logit 与 stable softmax 用 warp reduction 并行。长上下文版本按 32-token tile、256-token partition 计算局部 FP32 `(m,l,o)`，再由第二个 kernel 合并 online-softmax state；生产 capability 扩到 context≤2048，未支持 shape 由上层 gate fail closed。
...
- 能力门禁：Remap 只在真实 donor prefix 可执行时开放；Paged 只支持 Qwen2.5-0.5B、FP16 KV、page 16、D64、每序列单 token、context ≤ 2048、完整 GPU offload，默认关闭。它不再把“单 token decode”错误限制为 batch 1：逐序列 block table/context、CPU reference、CUDA grid 与 scratch 已覆盖 batch 1/2/4/8。v2.10 的 K2/K1 性能晋级只覆盖 context17/batch 1，H10/H13 的 Paged/Direct 结果也仍是 batch 1；超出 capability envelope 的请求回退 Recompute。
- 验收入口：`scripts/run_issue7_acceptance.ps1` 串行执行 Direct/Paged differential、真实 Remap、KV pressure fallback、晚到 CUDA failure/recovery；CUDA build gate 为 K1–K5 各启独立进程，分别执行 24 个 batch-1 边界与 unified/non-unified 各 batch 2/4/8，以及异构 512/513 边界，共 32 个 Paged Flash Attention CPU/CUDA case。`scripts/run_paged_batch_acceptance.py` 另用默认 non-unified 的真实 `llama-server -np 8`、8 个不同 resident prefix 和并发 barrier；最终形成一个 batch=8 graph，CUDA 后端实际记录 24 个逐层 Paged dispatch 与 192 个 sequence-layer dispatch，零 fallback。Direct/Paged 的输出与 cache length 逐项一致，top-64 最少重叠 55、共同 token 最大 logprob 误差 0.6900，低于显式 48/1.0 功能验收门；该服务分布门不是预注册性能或精度研究结论，主要数值正确性仍由逐算子 CPU/CUDA oracle 负责。完整 runtime bundle、模型、源码 overlay 和原始日志均写入 `results/acceptance/paged-batch.json`。
- 批量性能边界：H19 不再把并发 barrier 数量直接叫作 kernel batch，而使用原生多 prompt 请求、逐 cell 记录 Paged graph 数，并把执行计数移到 CUDA kernel 内以覆盖 CUDA Graph replay。正式矩阵覆盖 batch 1/2/4/8、128/512/1024 token、6 个平衡匹配进程块。batch=8 主区间均实现 1 graph/wave、完整 24 层和零 fallback，但吞吐中位变化为 -3.22%，95% 区间 [-4.90%, -0.12%]；从原始 wave 样本重算的 Direct/Paged P95 为 131.459/152.495 ms，回退 16.00%；精确输出 token 1052/1080，故预注册 correctness、吞吐和尾延迟门均未通过。功能 batch 支持成立，性能 promotion 仍为 false。
- 混合路由晋级：NSYS 将 H19 根因定位为 custom K4 算术核约 40.54 μs、upstream Flash Attention 约 26.08 μs，而 merge 仅约 1.62 μs；partition/grid 减半和实验性 K5 均未改善。生产路由因此在 block-table 构建时识别物理连续性：连续页复用 upstream attention，碎片页才执行 K4。H20/v6.1 的 batch=8、128/512/1024-token、6-block 正式矩阵中，吞吐中位 +1.54%，cluster bootstrap 95% 区间 [-2.06%, +3.86%]，P95 从 142.532 ms 降到 141.449 ms（-0.76%），最差 cell 中位回退 +12.05%，1728/1728 输出一致；216 次 fast-path/1728 条序列且 custom graph/CUDA dispatch 为 0，预注册非劣门通过。结论仅是连续布局混合路由的有界正结果，H19 的碎片页 custom K4 负边界继续有效。
...
- 正式 K2 替换证据：v2.10 在干净提交 `a502764`、vendor `29da4f0` 上完成 30 组同进程、随机顺序的稳态 K1/K2 配对；每 variant 保留 480 条请求级计时与响应且逐项输出一致，累计 600 次 production Paged graph entry、fallback 0。请求级 median 为 6.900/6.938 ms（K2 回退 0.55%，配对簇 bootstrap 回退 95% 上界 2.86%），P95 为 30.102/30.559 ms（回退 1.52%）；PID 隔离 NSYS 的相同 480 次目标 kernel 总时长为 8.174/4.051 ms（-50.44%）。K2 通过预注册的 Paged 内部 bounded-replacement 门槛。v2.0/v2.1/v2.5/v2.9 为有效失败，v2.3/v2.4/v2.6/v2.7/v2.8 因能力门、时间线或指标语义问题无效/中止并继续留档；该结果不改写 Paged-vs-Direct 的负结论。
- 长上下文 H10→H13 证据链：H10 的 split-K2 在 512–2048 token 上中位回退 +50.35%，由此定位标量 QK/PV、两-head tile 重复装载 GQA K/V 与固定分区问题。K4 改为 256-thread CTA 覆盖完整 GQA7、`half2` 向量访问和设备端 64/128-token 自适应 partition；24 个 production GGML CPU/CUDA oracle case 覆盖 1–2048 token、页/tile/partition 边界和反序物理页。最终 H13 从 3 份当前仓库文档构造 18 个来源/hash 绑定 prompt，以 6/6 平衡臂顺序的 12 个匹配进程块采集 432 个 workload-arm、3456 个测量请求，输出一致且 0 fallback。主中位回退降至 +3.98%，cluster bootstrap 95% 区间 [+2.50%, +5.38%]，P95 +13.34%，最差 workload +8.76%；因 CI 上界仍高于 +5% 而保持 opt-in。H14 的 64-token tile 结果未改善并已回退。
...
- v1.4.0 正式 CUDA matched-workload replay 保存并语义校验 120 个隔离 trace、600 组原始 Prometheus/响应 observation；80 个训练 trace 按时间拆为 60 个拟合与 20 个单侧校准，40 个 evaluation trace 始终留出。H0/A1/L1/D1 的 median/P95/cumulative regret 均为 0/3.969/58.142 ms、harmful 0，D1 因置信上界非负而 0 次切换，未通过最少切换及负 regret CI 门禁。离线 T1 为 0/3.258/34.938 ms，paired trace-cluster mean-regret delta 95% CI 为 [-0.4687, -0.1341] ms，但 15 次切换出现 1 次 harmful（1.25%），因此同样不晋级。结论是当前全局线性差值模型安全但欠拟合，而不是学习策略已证明有效；v1.4.0 作为负结果完整保留，后续结构调整必须使用新协议和新留出数据。
- v1.5.0 使用 120 个全新隔离 trace 验证预注册的 512-token 分段 D2。H0 与 D2 的 median/P95/cumulative regret 均为 0/3.297/44.958 ms、harmful 0；低上下文 Recompute 的 95% 单侧校准偏移为 7.446 ms，D2 因此 0 次切换并失败。T1 为 0/1.763/23.138 ms、CI [-0.5216, -0.0663] ms，但 15 次切换产生 3 次 harmful。v1.4 开发回放中的 D2 改善没有被冒充为新数据结论，当前生产选择仍是 H0。
- v1.6.0 用新的 collection/bootstrap seed 和 120 个全新隔离 trace 验证风险预算 D3。D3 在 80 个 evaluation 决策中切换 24 次，累计 regret 2.082 ms，相对 H0 42.590 ms 下降 95.1%；P95 0 ms，相对 H0 3.169 ms；matched-workload trace-cluster CI [-0.7232, -0.3069] ms。1 次 harmful 按协议中全部 eligible decisions 的分母为 1.25%，按实际 switches 分母为 4.17%，两种本次 replay 的经验比例均低于 5%；累计收益 41.480 ms 大于累计损失 0.972 ms，所有门禁通过。这里没有用 harmful proportion 的置信上界，因此不声称总体风险以 95% 置信度低于 5%。四个 action 由独立固定模式进程采集，action×process 混杂未消除；JSON 中 `paired_trace_cluster...` 是遗留字段名，不是 CONTEXT 定义的 Trial Pair。该结果只证明 matched-workload replay 的预测选择有效并授权同进程 monitored canary，不等于动作因果效应、已经在线启用或跨模型/跨 GPU 泛化。
