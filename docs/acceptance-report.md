...
K2 针对生产 `14Q/2KV/D64`、context≤32 的 GQA7 envelope，把 K1 的每 query-head CTA 改为每 CTA 两个 query-head warp；每个 CTA 共享装载其两个 query heads 的 K/V，使每 KV head 的装载 tile 从 7 个降为 4 个。K 采用按维度转置的 shared-memory 布局，logit 与 stable softmax 用 warp reduction 并行。生产默认 dispatch K2；K1 由实验 selector 保留为 incumbent，并在 context>32 时作为 kernel 内完整写回路径，未支持 shape 由上层 capability gate fail closed。
...
- 能力门禁：Remap 只在真实 donor prefix 可执行时开放；Paged 只支持 Qwen2.5-0.5B、FP16 KV、page 16、D64、单 token/batch 1、context ≤ 32、完整 GPU offload，默认关闭；正式 K2 性能晋级只覆盖 context17。超出 capability envelope 的受控用户请求回退 Recompute，不把 H3 microbenchmark 当作生产证据。
