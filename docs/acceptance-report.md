...
- 能力门禁：Remap 只在真实 donor prefix 可执行时开放；Paged 只支持 Qwen2.5-0.5B、FP16 KV、page 16、D64、单 token/batch 1、context ≤ 2048、完整 GPU offload，默认关闭；v2.10 的 K2/K1 性能晋级只覆盖 context17，H10 的 Paged/Direct 负结果覆盖 64–2048。超出 capability envelope 的受控用户请求回退 Recompute，不把 H3 microbenchmark 当作生产证据。
