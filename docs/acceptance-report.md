...
- 正式 K2 替换证据：v2.7 在干净提交 `4940853` 上完成 30 组同进程、随机顺序的稳态 K1/K2 配对，累计 300 次请求级 production Paged graph entry、fallback 0、输出一致。八 token decode 的客户端 median 为 44.879/41.417 ms（K2 -7.72%），P95 为 57.253/55.897 ms（-2.37%）；PID 隔离 NSYS 的相同 24 次目标 kernel 总时长为 0.438/0.228 ms（-47.90%）。K2 通过预注册的 Paged 内部替换门槛；v2.0/v2.1/v2.4/v2.5 失败结果与 v2.3/v2.6 无效实验继续留档，且该结果不改写 Paged-vs-Direct 的负结论。
