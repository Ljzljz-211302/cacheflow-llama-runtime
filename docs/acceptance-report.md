...
- 长上下文 H10 证据：split-K2 用 32-token tile、256-token partition 和 FP32 online-softmax merge 扩展到 context 2048；24 个 production GGML CPU/CUDA oracle case 覆盖页、tile、partition 边界和反序物理页。正式 v4 从 3 份真实仓库文档构造 18 个来源/hash 绑定 prompt，在 10 个随机化匹配进程块中采集 360 个 workload-arm 单元、1440 个测量请求，实际覆盖 64–2048 token、4–128 页，输出一致且 0 fallback。512–2048 token 的 `server_prompt_ms` 中位回退 +50.35%，cluster bootstrap 95% 区间 [+49.19%, +51.19%]，P95 回退 +63.74%，因此 2048 token 内无 Direct crossover，保持 opt-in。误选恒为 0.001 ms 的 `predicted_ms` 的 v3 完整移入 superseded 并标为无效。
