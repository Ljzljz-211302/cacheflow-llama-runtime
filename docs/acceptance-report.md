...
- 正式 K2 替换证据：v2.10 在干净提交 `a502764`、vendor `29da4f0` 上完成 30 组同进程、随机顺序的稳态 K1/K2 配对；每 variant 保留 480 条请求级计时并累计 600 次 production Paged graph entry，fallback 0、输出一致。请求级 median 为 6.900/6.938 ms（K2 回退 0.55%，配对簇 bootstrap 回退 95% 上界 2.86%），P95 为 30.102/30.559 ms（回退 1.52%）；PID 隔离 NSYS 的相同 480 次目标 kernel 总时长为 8.174/4.051 ms（-50.44%）。K2 通过预注册的 Paged 内部 bounded-replacement 门槛。v2.0/v2.1/v2.5/v2.9 为有效失败，v2.3/v2.4/v2.6/v2.7/v2.8 因能力门、时间线或指标语义问题无效/中止并继续留档；该结果不改写 Paged-vs-Direct 的负结论。
