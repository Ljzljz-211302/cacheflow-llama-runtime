...
- v1.6.0 用新的 collection/bootstrap seed 和 120 个全新隔离 trace 验证风险预算 D3。D3 在 80 个 evaluation 决策中切换 24 次，累计 regret 2.082 ms，相对 H0 42.590 ms 下降 95.1%；P95 0 ms，相对 H0 3.169 ms；paired trace-cluster CI [-0.7232, -0.3069] ms。1 次 harmful 对应 1.25%，低于预注册 5% 上限；累计收益 41.480 ms 大于累计损失 0.972 ms，所有门禁通过。该结果证明本 matched-workload replay 中风险约束学习策略有效，但只授权 monitored production canary，不等于已经在线启用或证明跨模型/跨 GPU 泛化。
