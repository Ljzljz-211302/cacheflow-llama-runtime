...
- 实现 Qwen2.5 Paged Decode K1/K2 CUDA kernel 与生产 dispatch：K2 用双 warp query-head tile 将每 KV head 的 K/V 装载由 7 次降为 4 次，并实现转置 shared K 与 warp stable-softmax；同进程 30 组×每臂 16 次请求中，480 个测量响应/variant 逐字一致，端到端 median/P95 保持在 0.55%/1.52% 回退内（median 回退 95% 上界 2.86%），NSYS 相同 480 次 kernel 总时长下降 50.44%，每 variant 累计 600 次真实 Paged graph entry 且 0 fallback，通过预注册生产替换门槛；同时保留 Paged 相对 Direct 未晋级、整体路径仍为 opt-in 的边界。
