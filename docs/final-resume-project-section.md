...
- 实现 Qwen2.5 Paged Decode K1 CUDA kernel 与生产 dispatch，覆盖跨页、非连续页和故障原子回退；10 组真实服务 AB/BA 输出逐字一致，Paged 进入 production graph 10 次、fallback 0，Nsight Systems 捕获 24 个模型层 kernel launch；独立 promotion gate 将未晋级路径隔离为 opt-in，默认服务保持 Direct。
