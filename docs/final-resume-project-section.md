...
- 实现 Qwen2.5 Paged Decode K1/K2 CUDA kernel 与生产 dispatch：K2 用双 warp query-head tile 将每 KV head 的 K/V 装载由 7 次降为 4 次，并实现转置 shared K 与 warp stable-softmax；同进程 30 组配对、300 次真实 Paged graph 均输出一致且 0 fallback，八 token 请求 median/P95 分别改善 7.72%/2.37%，NSYS 相同 24 次 kernel 总时长下降 47.90%；同时保留 Paged 相对 Direct 未晋级、整体路径仍为 opt-in 的边界。
