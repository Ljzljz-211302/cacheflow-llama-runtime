...
- 实现 Qwen2.5 Paged Decode K1/K2 CUDA kernel 与生产 dispatch：以双 warp query-head tile 将每 KV head 的 K/V 装载由 7 次降为 4 次，NSYS 相同 480 次 kernel 总时长下降 50.44%；进一步设计 32-token tile、256-token partition 与 FP32 online-softmax merge，将 production oracle 扩展到 2048 token。构建 3 个真实文档来源、18 个精确长度 workload、360-cell 的 Direct/Paged 客观矩阵，发现 512–2048 token 中位回退 50.35%（95% CI 49.19%–51.19%），据此保持 fail-closed/opt-in 并定位向量化 QK/PV 与空 partition 调度瓶颈。
