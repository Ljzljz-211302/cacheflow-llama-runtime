...
- 建立无 profiler 主结果与 Nsight Systems 机制证据分离的 CUDA 因果链：aligned 1-block CUDA-event 改善 57.10%（95% CI 38.51%–57.14%），misaligned 反例回退 137.94%；scalar/vector launch 数均为 10/10，否定“减少 launch”解释，并如实保留 NCU driver/tool 不兼容与计数器权限不足边界。
- 实现受限 Qwen2.5 Paged Decode K1 CUDA kernel 与生产 dispatch、跨页/非连续页和故障原子回退；v1.1 真实服务中 Paged P95 相对 Direct 回退 6.78%，因此 promotion gate 失败并保持 opt-in，不把可执行路径包装成默认加速。
- 设计 Direct/Remap/Paged/Swap/Recompute 统一候选接口及 capability/resource gate；Issue #6 的正式 H0/A1/T1/L1 replay 仅覆盖 Direct、device/host Swap、Recompute 的 200 条观测，另完成 500 万次零分配 chooser 开销微基准；T1 matched-workload paired CI 为 −0.5285～−0.0511ms，不将非克隆服务器状态包装为因果反事实。
