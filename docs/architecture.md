...
`run_benefit_gating_ab.py` 在相同CacheFlow运行栈中只改变门控策略，并从paired upstream/always/rule构造trace-level oracle。实验把CPU/CUDA与upstream/always/rule/learned笛卡尔积组成8个联合treatment，16个trial形成两个完整八阶Williams blocks：每个联合treatment在8个真实进程位置各出现2次，56条有向直接前驱也各出现2次；trial边界插入1秒washout，首处理的前驱明确记为 `trial_washout`。原始行保存完整 `treatment_order`、`process_position`、`previous_treatment` 与 `trial_washout_ms`，使真实机器顺序可审计。每波请求的one-shot guard覆盖 `HTTPConnection.request()` 的真实connect/body-send seam；响应等待与SSE消费发生在guard外，因而请求仍重叠执行。结果保留planned/observed send order；本地ThreadingHTTPServer测试强制首worker在send前让出40 ms，仍从服务端断言 `0..5` 到达。比较使用同一trial内的paired ratio，禁止用两个独立中位数相除。

生产 `server_benefit_policy::choose()` 由 `steady_clock` 原位计时并导出标准Prometheus histogram（1/2/5/10/20/50/100/250/+Inf us，同时有count/sum）；短程验收要求每个learned进程存在真实chooser样本，且最坏trial P99不超过预注册50 us。若learned实际执行CacheFlow，paired median regression仍必须不超过3%；若完全零干预，跨独立进程的wall-clock差不能识别CacheFlow因果效果，只能标记 `inconclusive_no_intervention`，并且仍须通过chooser P99、20% oracle-regret及harmful-trace非探索错误启用率不超过20%的门槛。风险预算按backend隔离：CPU可有限探索；已知always有害的CUDA fresh process将最小样本提高到64，在12-request trace内零probe/fail-closed，CUDA探索能力由长驻实验验证。
...
最终 53-wave CUDA 结果为：冷启动 CacheFlow/positive 均为 0；稳定阶段 exploration 26、positive-lower-bound 125，positive 覆盖 36 waves、最长及终端连续均为 35 waves，最后一次上下文 gauge 的预测收益/不确定性为 8.926/4.463 ms；分布切换 CacheFlow/positive 均为 0、安全回退 3。门禁要求至少连续 3 个 positive wave，且稳定阶段末尾仍连续存在；不以全程最大值或最后一个异构上下文的单值 gauge 制造选择偏差。CSV 逐wave保存每个请求的TTFT向量；`validate_long_lived_artifact()` 从原始counter/TTFT重算phase P95、positive wave、最大/终端streak和最终acceptance，运行脚本写出后立即自校验，普通测试还覆盖raw counter与copied acceptance篡改。
...
| Paged | 是（opt-in 受限 envelope） | 是 | Qwen2.5-0.5B、FP16 KV、D64、page 16、batch/query token 1、context ≤ 2048、全层 CUDA；v2.10 的 K2/K1 替换仅覆盖 context17，H10 另行覆盖 64–2048 的 Paged/Direct 负结果 |
...
`llama_context` 在 `mctx->apply()` 前完成 envelope 与布局检查。只有 Qwen2.5-0.5B `24L/14Q/2KV/D64`、page 16、FP16 K/V、非转置 V、每个活跃 sequence 恰有一个 query token、各 context ≤ 2048、causal Flash Attention、无 ALiBi/softcap/sink、完整 GPU offload 才允许 Paged 动作；其余请求保持 Direct/Recompute。布局把 block table 建模为 `[sequence][logical_block]`，context length 为逐序列向量，并额外标记所有活跃逻辑页是否按 page size 物理连续。连续布局复用 upstream attention，碎片布局才设置 custom Paged graph topology。CPU 用 `iq1+neq1*iq3` 选择 row，CUDA 根据 batch 位于 dim1 或 dim3 选择 Q/output stride，并为 non-unified K/V 加 stream stride。生产 K4 算子用一个 256-thread CTA 覆盖完整 7:1 GQA 组，以 `half2` 只加载一次 K/V，并依据实际 context 统一选择 64/128-token host/kernel partition。K1–K5 各自的 32 个 CPU/CUDA case 覆盖 24 个 batch-1 边界、unified/non-unified batch 2/4/8 及异构 512/513 边界；真实默认 non-unified 服务的强制 custom 验收形成一个 batch=8 graph，并由 CUDA 后端计数证明 24 个逐层 dispatch/192 个 sequence-layer dispatch，输出、cache length 和受限 top-64 分布门均通过。v2.10 与 H10→H13 的性能结论仍是 batch 1；批量正确性不改写这些历史性能边界。详见 [ADR 0002](adr/0002-sequence-aware-paged-decode-batching.md) 与 [ADR 0003](adr/0003-paged-hybrid-execution.md)。

H19 修正了这里的可观测性语义：host 侧 `ggml_cuda_paged_fattn()` 调用只代表普通提交或 CUDA Graph 捕获，graph replay 不会再次经过 host，因而不能充当“实际执行次数”。K1–K5 主 kernel 现由 block 0/thread 0 在设备端各递增一次 dispatch 与 batch-size counter，replay 会真实执行该原子操作；Prometheus 读取设备值后，才可验证 `kernel executions × sequences`。实验同时区分 requested batch 与 realized graph batch：主 batch=8 强制一波一图；较小 batch 保留生产调度器可能分图的事实并报告 `paged_sequences/paged_calls`，不能把 HTTP 数量硬编码成 GPU batch。H20 又把“custom kernel 执行”与“连续布局 fast-path”拆成两组互斥计数；因此连续布局走 upstream attention 时不会虚报 Paged CUDA dispatch，验证器要求 fast-path calls/sequences 精确匹配且 custom graph/CUDA 均为零。
...
K2 的正式 `h8-k2-production-v2.10.0` 工件从干净外层提交 `a502764`、vendor 提交 `29da4f0` 运行 30 组同进程随机 K1/K2 配对。每 arm 先清空 slot，执行 1 次完整 prompt 与 4 次 cached warm request，再记录 16 次 context-17 cached one-token 请求；每 variant 共 480 条无 profiler 测量响应且逐项输出一致，累计 600 次 Paged graph entry、fallback 0。K2 相对 K1 的请求级 median 为 6.900/6.938 ms（回退 0.55%，配对簇 bootstrap 回退 95% 上界 2.86%），P95 为 30.102/30.559 ms（回退 1.52%）；NSYS 中相同 480 次目标 kernel 总时长由 8.174 ms 降至 4.051 ms（-50.44%）。因此 K2 通过“median 回退 95% 上界≤5% + P95 点估计回退≤5% + kernel 总时长降低≥20%”的预注册替换门槛。此前有效失败与无效/中止实验均留档；v2.10 只晋级 context 17 重复 cached decode 的 Paged 内部 K2，不改变 Paged-vs-Direct 的 opt-in 结论，也不把 P95 bootstrap 描述为总体尾延迟非劣证明。
