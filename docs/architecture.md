...
`llama_context` 在 `mctx->apply()` 前完成 envelope 与布局检查。只有 Qwen2.5-0.5B `24L/14Q/2KV/D64`、page 16、FP16 K/V、非转置 V、单 sequence/单 query token、context ≤ 2048、causal Flash Attention、无 ALiBi/softcap/sink、完整 GPU offload 才设置 Paged graph topology；其余请求保持 Direct/Recompute。生产 K4 算子用一个 256-thread CTA 覆盖完整 7:1 GQA 组，以 `half2` 只加载一次 K/V，并依据设备端实际 context 选择 64/128-token partition；第二个 kernel 用 FP32 online-softmax state merge 合并 partition。CPU oracle 覆盖 1/15/16/17/31/32/33/63/64/65/127/128/129/255/256/257/511/512/513/1023/1024/1025/2047/2048，并对多页用例反转物理页序；v2.10 的 K2/K1 性能结论仍只覆盖 context17。长上下文 H10→H13 证据链覆盖 64–2048：K4 将旧 K2 的主中位回退从 +50.35% 降至 +3.98%，但置信上界 +5.38% 未过 +5% 门，仍不晋级。graph reuse key 包含 Paged topology，避免复用错误图。
