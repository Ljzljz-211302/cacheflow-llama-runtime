...
`llama_context` 在 `mctx->apply()` 前完成 envelope 与布局检查。只有 Qwen2.5-0.5B `24L/14Q/2KV/D64`、page 16、FP16 K/V、非转置 V、单 sequence/单 query token、context ≤ 2048、causal Flash Attention、无 ALiBi/softcap/sink、完整 GPU offload 才设置 Paged graph topology；其余请求保持 Direct/Recompute。生产 GGML 算子以 32-token tile 和 256-token partition 处理长序列，第二个 kernel 用 FP32 online-softmax state merge 合并 partition。CPU oracle 覆盖 1/15/16/17/31/32/33/63/64/65/127/128/129/255/256/257/511/512/513/1023/1024/1025/2047/2048，并对多页用例反转物理页序；v2.10 的 K2/K1 性能结论仍只覆盖 context17，H10 的 Paged/Direct 结论覆盖 64–2048 且为未晋级负结果。graph reuse key 包含 Paged topology，避免复用错误图。
