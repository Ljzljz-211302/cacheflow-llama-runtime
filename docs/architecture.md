...
`llama_context` 在 `mctx->apply()` 前完成 envelope 与布局检查。只有 Qwen2.5-0.5B `24L/14Q/2KV/D64`、page 16、FP16 K/V、非转置 V、每个活跃 sequence 恰有一个 query token、各 context ≤ 2048、causal Flash Attention、无 ALiBi/softcap/sink、完整 GPU offload 才设置 Paged graph topology；其余请求保持 Direct/Recompute。布局把 block table 建模为 `[sequence][logical_block]`，context length 为逐序列向量；CPU 用 `iq1+neq1*iq3` 选择 row，CUDA 根据 batch 位于 dim1 或 dim3 选择 Q/output stride，并为 non-unified K/V 加 stream stride。生产 K4 算子用一个 256-thread CTA 覆盖完整 7:1 GQA 组，以 `half2` 只加载一次 K/V，并依据设备端实际 context 选择 64/128-token partition。每个 K1–K4 variant 的 30 个 CPU/CUDA case 覆盖 24 个 batch-1 边界及 unified/non-unified 各 batch 2/4/8；真实默认 non-unified 服务形成一个 batch=8 graph，并由 CUDA 后端计数证明 24 个逐层 dispatch/192 个 sequence-layer dispatch，输出、cache length 和受限 top-64 分布门均通过。v2.10 与 H10→H13 的性能结论仍是 batch 1；批量正确性不改写这些历史性能边界。详见 [ADR 0002](adr/0002-sequence-aware-paged-decode-batching.md)。

H19 修正了这里的可观测性语义：host 侧 `ggml_cuda_paged_fattn()` 调用只代表普通提交或 CUDA Graph 捕获，graph replay 不会再次经过 host，因而不能充当“实际执行次数”。K1–K4 主 kernel 现由 block 0/thread 0 在设备端各递增一次 dispatch 与 batch-size counter，replay 会真实执行该原子操作；Prometheus 读取设备值后，才可验证 `kernel executions × sequences`。实验同时区分 requested batch 与 realized graph batch：主 batch=8 强制一波一图；较小 batch 保留生产调度器可能分图的事实并报告 `paged_sequences/paged_calls`，不能把 HTTP 数量硬编码成 GPU batch。
