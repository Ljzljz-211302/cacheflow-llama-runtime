...
这里的 batch 必须区分两个概念。“每序列单 token decode”是 Transformer 自回归阶段的语义：每条活跃请求本轮只产生一个新 token；“batch=1”则表示整个 GPU 图只有一条请求。旧实现错误地把两者绑定。修复后 block table 变为 `[sequence][logical_block]`，CPU row 使用 `iq1+neq1*iq3`，CUDA 同时识别 unified `[D,B,H,1]` 与默认 non-unified `[D,1,H,B]` 的 stride，K/V stream 和 partition scratch 也逐序列隔离。K1–K4 各自通过 30 个 production oracle case；真实 batch=8 服务由后端实际记录 24 个 layer dispatch/192 个 sequence-layer dispatch，零 fallback，Direct/Paged 输出及 cache length 相同，top-64 最少重叠 55、最大 logprob 误差 0.6900（门限 48/1.0）。该结果是架构和已声明数值边界证据，不把既有 batch-1 性能负结果改写为批量加速。

面试追问“batch 大了是否自然更快”时，应沿完整证据链回答。第一，requested batch 是 API 一次交给服务的序列数，realized graph batch 是调度器实际放进同一计算图的序列数，两者可能不同；所以 H19 同时记录 graph calls、sequences 和设备端 kernel executions。第二，CUDA Graph 首次 capture 后会直接 replay，host dispatch 函数不会重入；因此旧 host counter 会漏数，项目把计数移入 kernel，才证明 batch=8 每波确实执行 24 层、每层 8 条序列。第三，功能可执行不等于性能占优：H19 的 batch=8 吞吐中位变化 -3.22%，95% 区间 [-4.90%, -0.12%]；从原始 wave 样本重算的 Direct/Paged P95 是 131.459/152.495 ms，回退 16.00%，且只有 1052/1080 个输出 token 精确一致。结论是“根架构已解除 batch=1 限制，但当前 K4 在该设备/模型/输入矩阵下不能晋级”，而不是为了简历把负结果改成加速。

继续追问“那如何解决”时，应回答布局感知混合执行，而不是继续调一个 tile 常数。NSYS 显示 H19 的 K4 主 kernel 平均约 40.54 μs，upstream Flash Attention 约 26.08 μs，merge 只有约 1.62 μs；所以瓶颈在自研算术主体。partition/grid 减半没有改善，实验性 Tensor Core K5 也更慢。根修复是让 block-table builder 判断活跃逻辑页是否物理连续：连续时保留 Paged 生命周期与能力门禁，但复用 upstream attention；只有碎片布局才承担页表寻址 K4 的代价。H20 在 batch=8、128/512/1024 token、6 个平衡匹配进程块中得到吞吐中位 +1.54%、95% 区间 [-2.06%, +3.86%]，P95 -0.76%，1728/1728 输出一致并通过非劣门。面试中必须说“连续布局混合路由通过有界非劣”，不能说“自研 K4 严格快于 Direct”。

证据链要分三层回答。第一层是正确性：生产算子用 CPU Flash Attention 在 context 1/15/16/17/18/31/32/33 八个能力与回退边界、确定性区分输入与非连续物理页上交叉验证；正式 K1/K2 的 480 个测量响应/variant 逐字一致，每 variant 累计 600 次请求级真实 Paged 图且 0 fallback。第二层是无 profiler 服务指标：同一长驻进程内随机执行 30 组配对，每 arm 保留 16 条原始请求计时，所以每 variant 有 480 个用户请求样本；请求级 median 为 6.900/6.938 ms（K2 回退 0.55%，配对簇 bootstrap 回退 95% 上界 2.86%），P95 为 30.102/30.559 ms（回退 1.52%）。第三层是机制：PID 隔离 NSYS 中两臂各 480 次目标 kernel，总时长 8.174→4.051 ms（-50.44%）。NSYS 证明执行的是哪个 kernel 以及其时间；因为 NCU hardware counter 不可用，不能声称已直接测得 DRAM byte、occupancy 或 bank-conflict counter。
...
如果追问“有没有公认数据集”，回答 H21：内容和标签来自 THUDM LongBench，生产到达/长度分别来自 BurstGPT v2.0 与 Azure LLM Inference Trace 2024，原文件 SHA-256、官方行号、prompt hash 和本地 tokenizer 长度全部冻结。两类数据并非同一业务事件，所以准确名称是 trace-driven public-content synthetic replay，而不是“真实联合生产流量”。性能覆盖两源各24请求、12个匹配进程块；质量覆盖 `multifieldqa_en/2wikimqa/triviaqa` 各2条完整未截断样本。LongBench-E 完整样本超过当前2048-token能力门，因此没有截断后冒充LongBench-E成绩；LMSYS-Chat-1M需要授权，本轮也没有声称使用。

公开数据的价值不仅是换 prompt。它暴露旧实现把一次性 KV action observation 当成整请求 Paged route：冷 prompt 路由为0，多 token请求只覆盖首个图；continuous batching 又会把 prefill/decode 或不同 action 混图并让 graph-wide Paged 静默回退；最初 8 个客户端 worker 还会反压 24 请求的到达过程。修复后 route state 持续到请求结束，按 phase+action 分图，每请求由独立定时客户端提交。v1.3 的12块共记录3192/3192生成decode序列进入hybrid、0 fallback，最大到达偏差5.605ms；吞吐中位-0.04%、95%区间[-1.54%,+0.87%]，P95回退1.19%、其区间[-1.51%,+3.13%]。LongBench 6/6输出和平均QA F1 0.391300完全一致，但trace只有565/576完整输出一致，所以严格晋级仍失败。面试中应把“修复真实生命周期与到达重放缺陷、公开数据外部复核、保留近似tie负结果”作为工程与科研贡献，不能说Paged严格更快或准确率100%。
