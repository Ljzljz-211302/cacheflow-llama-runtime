...
如果追问“有没有公认数据集”，回答 H21：内容和标签来自 THUDM LongBench，生产到达/长度分别来自 BurstGPT v2.0 与 Azure LLM Inference Trace 2024，原文件 SHA-256、官方行号、prompt hash 和本地 tokenizer 长度全部冻结。两类数据并非同一业务事件，所以准确名称是 trace-driven public-content synthetic replay，而不是“真实联合生产流量”。性能覆盖两源各24请求、4个匹配进程块；质量覆盖 `multifieldqa_en/2wikimqa/triviaqa` 各2条完整未截断样本。LongBench-E 完整样本超过当前2048-token能力门，因此没有截断后冒充LongBench-E成绩；LMSYS-Chat-1M需要授权，本轮也没有声称使用。

公开数据的价值不仅是换 prompt。它暴露旧实现把一次性 KV action observation 当成整请求 Paged route：冷 prompt 路由为0，多 token请求只覆盖首个图；continuous batching 又会把 prefill/decode 混图并让 graph-wide Paged 静默回退。修复后 route state 持续到请求结束，且 Direct/Paged 都按相同 phase 边界拆图，四块共记录1064/1064生成decode序列进入hybrid、0 fallback。v1.2吞吐中位-0.34%、95%区间[-0.79%,+0.14%]，P95回退12.18%；LongBench 6/6输出和平均QA F1 0.391300完全一致，但trace只有191/192完整输出一致，所以严格晋级仍失败。面试中应把“修复真实生命周期缺陷、公开数据外部复核、保留一条近似tie负结果”作为工程与科研贡献，不能说Paged严格更快或准确率100%。
