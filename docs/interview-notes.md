...
面试追问“batch 大了是否自然更快”时，应沿完整证据链回答。第一，requested batch 是 API 一次交给服务的序列数，realized graph batch 是调度器实际放进同一计算图的序列数，两者可能不同；所以 H19 同时记录 graph calls、sequences 和设备端 kernel executions。第二，CUDA Graph 首次 capture 后会直接 replay，host dispatch 函数不会重入；因此旧 host counter 会漏数，项目把计数移入 kernel，才证明 batch=8 每波确实执行 24 层、每层 8 条序列。第三，功能可执行不等于性能占优：H19 的 batch=8 吞吐中位变化 -3.22%，95% 区间 [-4.90%, -0.12%]，P95 wave latency 回退 36.67%，且只有 1052/1080 个输出 token 精确一致。结论是“根架构已解除 batch=1 限制，但当前 K4 在该设备/模型/输入矩阵下不能晋级”，而不是为了简历把负结果改成加速。
