...
v1.6 的全新留出数据通过了全部预注册门禁：D3 产生 24 次切换，累计 regret 从 42.590 ms 降至 2.082 ms，P95 从 3.169 ms 降至 0，matched-workload trace-cluster 95% CI 为 [-0.7232, -0.3069] ms；harmful 按全部 eligible decisions 为 1/80（1.25%），按实际 switches 为 1/24（4.17%），41.480 ms 累计收益覆盖 0.972 ms 累计损失。5% gate 是本次 replay 的经验比例门槛，并未对 harmful proportion 构造置信上界。四个 action server 是独立进程，因而这里不是 Trial Pair，不能排除 action×process 混杂。该结论的边界是单模型、单 GPU、matched-workload 离线 replay；生产上线仍需独立同进程 canary。
