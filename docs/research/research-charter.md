...
Issue #5 的 H3 v1.0.0 已得到受限负结果：K1 直接分页 kernel 通过 D64/D128 correctness gate，但相对相同数学的 contiguous CUDA 对照，9 个预注册 regime 的 GPU-event 中位数全部回退 6.25%–13.06%。0.5B/7B-shape 在 context 1024、batch 1 分别回退 13.06% 与 11.59%；对应 batch-1/batch-4 差只有 0.99/1.82 个百分点，不支持优先做 split-K。预注册规则因此选择 K2 GQA KV reuse 作为下一可证伪假设。4 个方法隔离 NSYS capture 各有 5 次目标 launch；NCU 不完整，禁止 memory/occupancy 机制归因。该结果不表示 K2 已实现，也不表示 0.5B 生产请求已经接入。
