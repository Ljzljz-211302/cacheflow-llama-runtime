...
Issue #6 的 v1.0.0 至 v1.2.0 因证据合同问题否决，v1.4/v1.5 的 D1/D2 负结果也完整保留。v1.6.0 使用新的 collection/bootstrap seed、120 个全新隔离 trace、600 组原始观测和 60 fit/20 calibration/40 evaluation 时间拆分验证风险预算 D3。D3 24 次切换，累计 regret 2.082 ms，相对 H0 42.590 ms 下降 95.1%；P95 0 对 3.169 ms；paired trace-cluster CI [-0.7232, -0.3069] ms；harmful 1/80（1.25%）低于 5% 预算，累计 gain/harm 为 41.480/0.972 ms，全部门禁通过。动作服务器并非克隆同一内存状态，因此只称 matched-workload replay 中有效并授权 monitored canary，不作生产在线或跨环境因果收益声明。
