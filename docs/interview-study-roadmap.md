...
- 短程 gating：CPU regression −12.46%、oracle regret 16.34%；CUDA −6.17%、2.44%，CUDA fresh-process 0 probe。
- CUDA causal：decision +15、chunk +23、prefill token −145、copy −2.519MB、Event −0.260ms、Engine +31.23ms、TTFT P95 +44.79ms。
- 最终严格入口：`verify.ps1 -Full`，1224.9 秒，退出 0；51 个 Python 测试及原生/CUDA/兼容/故障/模型/性能门禁通过。
