...
- 当前实现包含 KV 数据移动算子与受限 Qwen2.5-0.5B Paged Decode K1/K2；host 正确性能力门覆盖 D64/GQA7/context≤32，而 K2 正式性能晋级只覆盖 page16/context17 的重复 cached 请求，它不是通用 FlashAttention、GEMM、prefill PagedAttention 或任意模型实现；
