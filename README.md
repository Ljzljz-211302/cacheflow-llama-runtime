...
已执行研究工作并非只有 Issue #7：Issue #2/#3 固定可证伪问题与预注册协议，Issue #4 建立 CUDA/NSYS 因果链，Issue #5 实现受限 Paged Decode kernel，Issue #6 完成五动作统一接口与 H0/A1/T1/L1 artifact，Issue #10 审计一手资料与可复现基线。完整映射见 [严格验收报告](docs/acceptance-report.md#已执行-issue-覆盖)。Issue #8/#9 仍是待完成的消融、外部有效性和最终论文式发布，不计入当前成果。
