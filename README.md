...
- 服务级 NSYS 关联：3 个 no-profiler pairs 与相同 seed/config 的 3 个 profiler pairs 分离，6 个 server PID、72 个 request ID 逐项连接 scheduler decision、KV action、NSYS CUDA kernel 与 TTFT，44 次 profiler replay KV launch 和逐进程运行时 counter 完全一致。本轮 no-profiler 中 decision +22、prefill chunk +32、prefill token -774、KV launch +2、copy bytes +9,904,200、CUDA Event -0.293 ms；TTFT P95/Engine execute 中位差为 +108.810 ms/+181,209 us，两者同向恶化。报告不冒充复现了旧实验的反号结果。
