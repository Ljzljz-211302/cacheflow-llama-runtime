...
- Conservative Benefit Gating：最终 CPU/CUDA 各 10-trial Latin 验收在真实 socket send seam 固定每波顺序并保持响应/SSE 重叠，结果同时保存 planned 与 observed send order；CPU learned objective median 4153.18 ms、paired upstream regression -25.87%、paired-oracle regret 3.48%，CUDA 223.23 ms、paired regression +1.64%、paired-oracle regret 3.92%。两端均通过原 3%/20% 与 harmful wrong-enable 门槛；CUDA 的 6 个 harmful trials 中错误启用为 0。短生命周期采用 backend-local 风险预算：CPU 26 次有限探索，CUDA 0 次 probe 并 fail closed；positive-lower-bound 均为 0，不把冷启动门禁冒充在线收敛。
