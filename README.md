...
H21 将输入替换为公认公开数据：LongBench 完整 QA 文本/标签、BurstGPT v2.0 与 Azure 2024 到达和 token trace。公开 workload 暴露出旧路由只维持一个 decode 图、且 mixed prefill/decode 会让整图静默退回 Direct；修复为 request-scoped route 状态并让两臂采用相同 phase 图边界后，四块正式实验记录 1064/1064 个生成 decode 序列进入连续布局 hybrid、0 fallback。v1.2 吞吐中位 `-0.34%`、block-cluster bootstrap 95% 区间 `[-0.79%, +0.14%]`，请求 P95 `2188.215/2454.645 ms`（`+12.18%`）；LongBench 6/6 输出及 QA F1 一致，但 trace 完整输出仅 `191/192`，故严格门仍 **FAIL**，不宣称 Paged 已晋级。数据处理、绝对质量分数和两次根修复见 [H21 官方数据报告](docs/research/public-workload-results.md)。
