...
正式 Qwen2.5-0.5B CUDA v1.6 matched-workload replay 使用 120 个全新隔离 trace（60 fit/20 calibration/40 evaluation）和 600 组 hash-bound 原始 Prometheus/响应 observation。风险预算 D3 在 80 个留出决策中切换 24 次：累计 regret 从 H0 的 `42.590 ms` 降至 `2.082 ms`（下降 95.1%），P95 从 `3.169 ms` 降至 `0`，paired trace-cluster mean-regret delta 95% CI 为 `[-0.7232, -0.3069] ms`；1 次 harmful（1.25%）低于预注册 5% 预算，累计收益 `41.480 ms` 覆盖累计损失 `0.972 ms`。D1/D2 仍 0 次切换，T1 为 `18.061 ms` 且同样有 1 次 harmful。D3 的通过只授权后续 monitored canary，当前生产 selector 仍不自动启用学习策略。决策开销基准 p99 0.900 us、decision/action ratio p99 0.0874%、零 allocation；该开销来自现有有界生产 chooser seam，D3 仍是离线候选。v1.4/v1.5 负结果均完整保留，未被成功版本覆盖。
