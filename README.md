...
长上下文版本把 K2 改为 32-token tile、256-token partition，并在第二个 CUDA kernel 中合并 FP32 online-softmax 状态；24 个 CPU/CUDA oracle 边界用例覆盖 1–2048 token 和反序物理页。H10 从 3 份真实项目文档生成 18 个来源绑定 prompt，精确覆盖 64/128/256/512/1024/2048 token；10 个随机化匹配进程块共形成 360 个 workload-arm 单元和 1440 个测量请求。512–2048 token 的 `server_prompt_ms` 中位回退为 **+50.35%**，process-block cluster bootstrap 95% 区间 **[+49.19%, +51.19%]**，P95 回退 **+63.74%**，到 2048 token 仍未出现 Paged 优于 Direct 的交叉点。Paged 因此继续默认关闭。Direct/Paged 共用同一底层分配器，当前 A/B 不支持容量或碎片优势主张。数据与算法说明见 [长上下文 Paged Attention 报告](docs/research/long-context-paged-attention.md)。
