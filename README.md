...
- 服务级 NSYS 关联：3 个 no-profiler pairs 与相同 seed/config 的 3 个 profiler pairs 分离，6 个 server PID、72 个 request ID 逐项连接 scheduler decision、KV action、NSYS CUDA kernel 与 TTFT，38 次 KV launch 和运行时 counter 完全一致。本轮 TTFT P95/Engine execute 中位差为 +346.82 ms/+258,596 us，两者同向恶化；报告不冒充复现了旧实验的反号结果。
