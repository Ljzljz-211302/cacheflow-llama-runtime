...
> Paged 性能不再由单一硬编码 prompt 决定：H9 使用独立冻结的六类 prompt 语料、30 组随机化匹配进程块和360个 workload-arm 观测，并按实际token/page分层。总体中位数显示改善，但 block-workload 回退分布P95与最差workload门禁失败，因此默认仍关闭；独立进程块不冒充共享热状态 Trial Pair。模型/vendor-diff 哈希是实验后的审计修正，不冒充运行前绑定。详见 [`h9-objective-paged-v2.0.0`](results/research/h9-objective-paged-v2.0.0/report.md)。
...
服务现在用同一个 fail-closed 接口比较 Direct、真实 CUDA Remap、受限 opt-in Paged Decode、CUDA-managed Swap、事务型 host Swap 与 Recompute。Paged 只开放 Qwen2.5-0.5B、FP16 KV、page 16、D64、每序列单 token、context≤2048 的全 GPU路径；布局、CPU reference 与 CUDA K1–K5 已加入独立 sequence 维，同时支持 unified `[D,B,H,1]` 与默认 non-unified `[D,1,H,B]`。每个 variant 的 32 个 CPU/CUDA case 覆盖 batch 1/2/4/8；真实 8 并发服务验收得到一个 batch=8 graph、24 个逐层 CUDA Paged dispatch/192 个 sequence-layer dispatch、零 fallback，Direct/Paged cache length 和输出逐项一致，top-64 最少重叠 55、最大 logprob 误差 0.6900（门限 48/1.0）。短上下文 K2 的正式替换证据仍只覆盖 context17，H10/H13 的 Direct/Paged 性能仍是 batch 1；功能批量化不冒充批量性能晋级。其他形状在 KV mutation 前回退。

H19 进一步把“请求数”与“实际 GPU graph batch”分开验收：8 条来源绑定输入、128/512/1024 token、请求 batch 1/2/4/8、6 个平衡匹配进程块，共 144 个 action-cell、1080 个测量输出。CUDA 计数已从 host 提交处移到 kernel 设备端，因此 CUDA Graph replay 也会被计入；batch=8 的全部主 cell 均为每波 1 graph、24 个 layer kernel、`24×8` 个 sequence-layer execution，零 fallback。客观结果没有晋级：batch=8 吞吐中位变化 `-3.22%`，进程块 cluster bootstrap 95% 区间 `[-4.90%, -0.12%]`；原始 wave 样本的 Direct/Paged P95 为 `131.459/152.495 ms`，回退 `+16.00%`；精确 token 为 `1052/1080`，因此 correctness 与性能门同时失败。Paged 仍为 opt-in，不能写成“大 batch 已优于 Direct”。正式原始数据、报告和图见 [H19 工件](results/research/h19-production-batched-paged-v5.0.0/report.md)。

H20 没有继续调常数掩盖 H19，而是依据 NSYS 根因重构生产路由：H19 的 custom K4 主 kernel 平均约 `40.54 μs`，upstream Flash Attention 约 `26.08 μs`，merge 仅约 `1.62 μs`；把 partition/grid 减半没有改善算术时间，实验性 Tensor Core K5 也更慢。现在 block-table builder 会识别物理连续页：连续布局复用 upstream attention，真正碎片页才进入 custom K4。预注册 v6.1 在 batch=8、128/512/1024 token、6 个平衡匹配进程块中得到吞吐中位 `+1.54%`、95% 区间 `[-2.06%, +3.86%]`，Direct/Paged P95 为 `142.532/141.449 ms`（变化 `-0.76%`），1728/1728 输出与 top-64 分布一致；216 次 fast-path 覆盖 1728 条序列，custom graph/CUDA dispatch 为 0，正式非劣门通过。该结果是连续布局的生产混合路由正结果，不证明碎片页 K4 严格优于 Direct，也不证明容量或碎片收益。见 [H20 工件](results/research/h20-paged-hybrid-batch8-v6.1.0/report.md)、[NSYS 根因留档](results/diagnostic/h20-batch8-context1024-nsys/README.md) 与 [ADR 0003](docs/adr/0003-paged-hybrid-execution.md)。

H21 将输入替换为公认公开数据：LongBench 完整 QA 文本/标签、BurstGPT v2.0 与 Azure 2024 到达和 token trace。公开 workload 暴露出旧路由只维持一个 decode 图、mixed prefill/decode 会让整图静默退回 Direct，以及 8 个客户端线程会反压 24 请求到达过程。修复为 request-scoped route 状态、按 phase+action 分图并使用每请求独立定时客户端后，v1.3 的 12 个匹配进程块记录 3192/3192 个生成 decode 序列进入连续布局 hybrid、0 fallback，最大到达偏差 5.605 ms。吞吐中位 `-0.04%`、block-cluster bootstrap 95% 区间 `[-1.54%, +0.87%]`；请求 P95 `2482.391/2511.827 ms`（`+1.19%`），其回退区间 `[-1.51%, +3.13%]`。LongBench 6/6 输出及 QA F1 一致，但 trace 完整输出仅 `565/576`，故严格门仍 **FAIL**，不宣称 Paged 已晋级。v1.2 因客户端到达反压保留为无效历史 replay。数据处理、绝对质量分数和根修复见 [H21 官方数据报告](docs/research/public-workload-results.md)。
...
在同一受限 Paged 路径内，K2 已替换逐 query-head CTA 的 K1：K2 以两个 warp 组成一个 query-head tile，每个 CTA 只装载一次其两个 query heads 共用的 K/V。对 GQA7 的每个 KV head，K1 需要 7 次装载，K2-T2 降为 4 个 tile/4 次装载；K 采用转置 shared-memory 布局，softmax 使用 warp reduction。正式 v2.10 在同一长驻服务进程内随机执行 30 组 K1/K2 配对，每 arm 完成 20 次真实 Paged 图并保留 16 条请求级计时；两臂各有 480 条无 profiler 测量响应且逐项输出一致，每 variant 累计 600 次 Paged graph entry、0 fallback。请求级 median 为 6.900/6.938 ms（K2 回退 0.55%，配对簇 bootstrap 的回退 95% 上界 2.86%），P95 为 30.102/30.559 ms（回退 1.52%）；PID 隔离 NSYS 中相同 480 次目标 kernel 总时长由 8.174 ms 降至 4.051 ms（-50.44%）。因此 K2 通过预注册的“median 回退 95% 上界≤5%、P95 点估计回退≤5%、kernel 总时长降低≥20%”替换门槛。该结论仅适用于 context17 的重复 cached decode，不推翻 Paged 相对 Direct 的负结果。

长上下文工作先用 H10 证明旧 K2 的根因不是 prompt：在 3 份真实项目文档、18 个精确长度 workload 上，512–2048 token 的 `server_prompt_ms` 中位回退为 **+50.35%**。随后重构为 K4：一个 256-thread CTA 覆盖完整 7:1 GQA 组，K/V 以 `half2` 只加载一次，并按上下文在设备端选择 64/128-token partition。24 个 CPU/CUDA oracle 边界用例覆盖 1–2048 token 和反序物理页。最终 H13 使用严格平衡的 6 组 Direct-first / 6 组 Paged-first、18 个来源绑定 prompt、432 个 workload-arm 单元和 3456 个测量请求；主中位回退降至 **+3.98%**，process-block cluster bootstrap 95% 区间 **[+2.50%, +5.38%]**，P95 **+13.34%**，最差 workload 中位 **+8.76%**。后三项中只有 CI 上界略超 +5% 门，因此 K4 不晋级、Paged 继续默认关闭。Direct/Paged 共用同一底层分配器，当前 A/B 不支持容量或碎片优势主张。数据与算法说明见 [长上下文 Paged Attention 报告](docs/research/long-context-paged-attention.md)。
