...
正式 Qwen2.5-0.5B CUDA v1.4 matched-workload replay 使用 120 个隔离 trace（80 train/40 evaluation），保存并语义校验 600 组原始 Prometheus/响应 observation，并把训练按时间再分为 60 个拟合 trace 与 20 个单侧校准 trace。H0/A1/L1/D1 的 median/P95/cumulative regret 均为 `0/3.969/58.142 ms`、harmful 0；D1 的分 regime 配对差值 Ridge 在加入机制交互、置信半径和校准偏移后仍未产生安全切换，因此不宣称学习策略加速。离线无安全门 T1 为 `0/3.258/34.938 ms`，paired trace-cluster mean-regret delta 95% CI `[-0.4687, -0.1341] ms`，但 15 次切换中有 1 次超过 H0 3%，故未通过安全晋级门禁。该负结果显示低/高上下文的 Recompute 成本存在非线性分段，而全局线性 D1 欠拟合；它被保留为后续分段策略的开发依据，不冒充确认性提升。决策开销 p99 0.500 us、decision/action ratio p99 0.0882%、零 allocation；源码审计仅确认未发现直接 CUDA 同步符号或后端 include，raw max 832.900 us 只作 Windows 抢占诊断。v1.0.0 至 v1.2.0 均仅保留在 `results/research/superseded/`，v1.3.0 保留为前序正式 replay。
...
正式 artifact 含 9 个 regime、360 条无 profiler paired observations 和 4 份方法隔离 NSYS trace。每个 regime 在计时前均由独立 CPU FP32 oracle 验证，最大绝对误差 `3.6e-8`。相对相同数学的 contiguous CUDA 对照，0.5B 的 16-token case 为 neutral，17-token case 出现高噪声的 +22.87% 中位改善但区间跨过材料性门槛，不能宣称稳定获益；所有 medium/long regime 则回退 10.41%–13.05%。本地 0.5B 的 context 1024/batch 1 回退 13.05%（95% CI 12.91%–13.12%），7B shape 对应回退 11.59%（11.37%–11.78%）。每份 NSYS capture 都严格包含 5 个目标 kernel launch。预注册 split-K 条件未触发，规则选择下一步验证 K2 GQA KV reuse；这只是待实现假设，不是加速结论。实验绑定 RTX 4050 `sm_89`、6141 MiB 显存，最大 benchmark device allocation 为 16,864,272 B。NCU 因 driver incompatibility 与 `ERR_NVGPUCTRPERM` 不完整，因此禁止 memory-bound、occupancy 和 DRAM-byte 归因。详见 [受限设计与一手资料](docs/research/restricted-paged-decode-attention.md)及[正式报告](results/research/h3-paged-decode-v1.0.0/report.md)。
