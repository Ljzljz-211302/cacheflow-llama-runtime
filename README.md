...
服务现在用同一个 fail-closed 接口比较 Direct、真实 CUDA Remap、受限 opt-in Paged Decode、CUDA-managed Swap、事务型 host Swap 与 Recompute。Paged 只开放 Qwen2.5-0.5B、FP16 KV、page 16、D64、batch 1、最多两个物理页（context≤32）的全 GPU路径；K2 的正式晋级证据覆盖 context 17--24，其他请求在 KV mutation 前回退。策略包含固定 H0、解析 A1、分桶查表 T1 与带置信边界/H0 fallback 的 L1，Prometheus 分开记录选择原因、实际动作、完整成本和失败次数。
...
在同一受限 Paged 路径内，K2 已替换逐 query-head CTA 的 K1：K2 以两个 warp 组成一个 query-head tile，每个 CTA 只装载一次其两个 query heads 共用的 K/V。对 GQA7 的每个 KV head，K1 需要 7 次装载，K2-T2 降为 4 个 tile/4 次装载；K 采用转置 shared-memory 布局，softmax 使用 warp reduction。正式 v2.7 在同一长驻服务进程内随机执行 30 组 K1/K2 配对，每 arm 经过 5 次真实 Paged 图，输出全部一致且 0 fallback。八 token decode 的客户端 median 由 44.879 ms 降至 41.417 ms（-7.72%），P95 由 57.253 ms 降至 55.897 ms（-2.37%）；PID 隔离 NSYS 中相同 24 次目标 kernel 总时长由 0.438 ms 降至 0.228 ms（-47.90%）。因此 K2 通过预注册的“median/P95 回退均不超过 5%且 kernel 至少降低 20%”替换门槛。结论仅适用于 Qwen2.5-0.5B、D64/GQA7、page16、context 17--24；它不推翻 Paged 相对 Direct 的负结果，也不外推到长 context。
