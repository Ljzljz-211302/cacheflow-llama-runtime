...
- Conservative Benefit Gating：最终 CPU/CUDA 各 10-trial Latin 验收在真实 socket send seam 固定每波顺序并保持响应/SSE 重叠，80/80 trial rows 的两波 observed order 均为 `0..5`；CPU learned objective median 4063.89 ms、paired upstream regression -24.87%、paired-oracle regret 5.59%，CUDA 232.46 ms、paired regression -1.93%、paired-oracle regret 1.73%。两端均通过原 3%/20% 与 harmful wrong-enable 门槛；CUDA 的 8 个 harmful trials 中错误启用为 0。短生命周期采用 backend-local 风险预算：CPU 19 次有限探索，CUDA 0 次 probe 并 fail closed；positive-lower-bound 均为 0，不把冷启动门禁冒充在线收敛。
