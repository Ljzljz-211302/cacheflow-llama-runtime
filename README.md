...
服务现在用同一个 fail-closed 接口比较 Direct、真实 CUDA Remap、受限 opt-in Paged Decode、CUDA-managed Swap、事务型 host Swap 与 Recompute。Paged 只开放 Qwen2.5-0.5B、FP16 KV、page 16、D64、batch 1、最多两个物理页（context≤32）的全 GPU路径；K2 的正式性能晋级证据只覆盖 context17 的重复 cached 请求，其他形状或性能范围不作外推并在能力不匹配时于 KV mutation 前回退。策略包含固定 H0、解析 A1、分桶查表 T1 与带置信边界/H0 fallback 的 L1，Prometheus 分开记录选择原因、实际动作、完整成本和失败次数。
