...
正式 Qwen2.5-0.5B CUDA v1.5 matched-workload replay 使用 120 个全新隔离 trace（80 train/40 evaluation），保存并语义校验 600 组原始 Prometheus/响应 observation，并把训练按时间再分为 60 个拟合 trace 与 20 个单侧校准 trace。H0/A1/L1/D1/D2 的 median/P95/cumulative regret 均为 `0/3.297/44.958 ms`、harmful 0；D2 虽按预注册的 512-token 边界分段，但低上下文 Recompute 的校准偏移达到 7.446 ms，因而 0 次切换并拒绝宣称加速。无安全门 T1 为 `0/1.763/23.138 ms`，paired trace-cluster mean-regret delta 95% CI `[-0.5216, -0.0663] ms`，但 15 次切换出现 3 次 harmful（3.75%），同样未晋级。v1.4 的开发回放曾显示分段 D2 可降低 regret，但 v1.5 新数据没有复现可安全切换结论，因此以 v1.5 的否决结果为准。决策开销 p99 0.500 us、decision/action ratio p99 0.0875%、零 allocation；源码审计仅确认未发现直接 CUDA 同步符号或后端 include。v1.0.0 至 v1.2.0 位于 `results/research/superseded/`，v1.3.0 与 v1.4.0 保留为前序审计记录。
