...
extern "C" {
    ...
    // Restricted decode-only paged KV mode. block_table contains physical KV
    // cell bases (one per logical block); context_lengths contains one length
    // per sequence. Backends that cannot honor this contract must reject op.
    GGML_API void ggml_flash_attn_ext_set_paged(
            struct ggml_tensor * a,
            struct ggml_tensor * block_table,
            struct ggml_tensor * context_lengths,
            int32_t              page_size);
