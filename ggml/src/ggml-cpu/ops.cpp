...
static int64_t ggml_flash_attn_ext_logical_kv_count(
        const ggml_tensor * dst, int64_t query_index) {
    if (dst->src[6] == nullptr) {
        return dst->src[1]->ne[1];
    }
    GGML_ASSERT(dst->src[5] != nullptr);
    GGML_ASSERT(dst->src[6]->type == GGML_TYPE_I32);
    GGML_ASSERT(query_index >= 0 && query_index < dst->src[6]->ne[0]);
    const int64_t count = ((const int32_t *) dst->src[6]->data)[query_index];
    GGML_ASSERT(count > 0 && count <= dst->src[1]->ne[1]);
    return count;
}

static int64_t ggml_flash_attn_ext_physical_kv_cell(
        const ggml_tensor * dst, int64_t query_index, int64_t logical_cell) {
    if (dst->src[5] == nullptr) {
        return logical_cell;
    }
    const int32_t page_size = ggml_get_op_params_i32(dst, 4);
    const int32_t * block_table = (const int32_t *) dst->src[5]->data;
    GGML_ASSERT(page_size > 0);
    GGML_ASSERT(query_index >= 0 && query_index < dst->src[5]->ne[1]);
    const int64_t physical_cell =
        block_table[query_index * dst->src[5]->ne[0] + logical_cell / page_size] +
        logical_cell % page_size;
    GGML_ASSERT(physical_cell >= 0 && physical_cell < dst->src[1]->ne[1]);
...
static void ggml_compute_forward_flash_attn_ext_f16_one_chunk(
    ...
    for (int ir = ir0; ir < ir1; ++ir) {
        ...
        const int64_t page_row = iq1 + neq1 * iq3;
        const int64_t logical_kv_count = ggml_flash_attn_ext_logical_kv_count(dst, page_row);
        const int64_t row_ic_end = dst->src[5] != nullptr ? logical_kv_count :
                std::min(ic_end, logical_kv_count);
        ...
        for (int64_t ic = ic_start; ic < row_ic_end; ++ic) {
            ...
            const int64_t physical_cell = ggml_flash_attn_ext_physical_kv_cell(dst, page_row, ic);
            const char * k_data = (const char *) k->data +
                (physical_cell*nbk1 + ik2*nbk2 + ik3*nbk3);
            ...
            const char * v_data = ((const char *) v->data +
                (physical_cell*nbv1 + iv2*nbv2 + iv3*nbv3));
...
static void ggml_compute_forward_flash_attn_ext_tiled(
    ...
    while (ir < ir1) {
        ...
        const int64_t page_row = iq1 + neq1 * iq3;
        const int64_t logical_kv_count = ggml_flash_attn_ext_logical_kv_count(dst, page_row);
        for (int64_t ic = 0; ic < logical_kv_count; ic += KV_TILE_SZ) {
            const int kv_tile = (int)std::min((int64_t)KV_TILE_SZ, logical_kv_count - ic);
            ...
            for (int tk = 0; tk < kv_tile; tk++) {
                const int64_t physical_cell =
                    ggml_flash_attn_ext_physical_kv_cell(dst, page_row, ic + tk);
                const char * k_data = (const char *)k->data +
                    physical_cell*nbk1 + ik2*nbk2 + ik3*nbk3;
...
static void ggml_compute_forward_flash_attn_ext_f16(
    ...
    const int64_t logical_kv_count = ggml_flash_attn_ext_logical_kv_count(dst, 0);
    const bool use_split_kv_path = !use_ref && (neq1 == 1 && neq3 == 1) && kv_is_f32_or_f16 && (k->type == v->type) && q->type == GGML_TYPE_F32 && logical_kv_count >= 512;

    if (use_split_kv_path) {
        const int64_t chunk_size = (logical_kv_count + nth - 1) / nth;
        ...
        bool use_tiled = !use_ref && dst->src[5] == nullptr &&
        ...
        while (current_chunk < nchunk) {
            ...
            if (use_tiled) {
                ...
                ggml_compute_forward_flash_attn_ext_f16_one_chunk(
                    params, dst, ir0, ir1, 0, logical_kv_count, nullptr, 0);
            }
