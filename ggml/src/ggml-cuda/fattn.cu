...
size_t ggml_cuda_flash_attn_ext_get_alloc_size(int device, const ggml_tensor * dst) {
    ...
    // Paged K1 needs no scratch beyond the ordinary output tensor, but the
    // buffer allocator still requires the output bytes themselves.
    if (dst->src[5]) return ggml_nbytes(dst);
    const ggml_tensor * K = dst->src[1];
    const ggml_tensor * V = dst->src[2];
...
void ggml_cuda_flash_attn_ext(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    if (dst->src[5]) {
        GGML_ASSERT(ggml_cuda_paged_fattn_supported(dst));
        ggml_cuda_paged_fattn(ctx, dst);
        return;
    }
...
bool ggml_cuda_flash_attn_ext_supported(int device, const ggml_tensor * dst) {
    if (dst->src[5]) return ggml_cuda_paged_fattn_supported(dst);
