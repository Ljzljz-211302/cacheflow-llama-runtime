...
//
static json server_kv_action_policy_to_json(const server_kv_action_policy_snapshot & snapshot) {
    json result = {
        { "decisions", snapshot.decisions },
        { "safe_fallbacks", snapshot.safe_fallbacks },
        { "invalid_feature_fallbacks", snapshot.invalid_feature_fallbacks },
        { "cold_start_fallbacks", snapshot.cold_start_fallbacks },
        { "uncertainty_fallbacks", snapshot.uncertainty_fallbacks },
        { "shadow_decisions", snapshot.shadow_decisions },
        { "total_decision_nanoseconds", snapshot.total_decision_nanoseconds },
        { "maximum_decision_nanoseconds", snapshot.maximum_decision_nanoseconds },
        { "reasons", json::object() },
        { "action_reasons", json::object() },
        { "observations", json::object() },
    };
    for (size_t action_index = 0; action_index < server_kv_action_count; ++action_index) {
        const auto action = static_cast<server_kv_action>(action_index);
        const char * action_name = server_kv_action_name(action);
        result[action_name] = snapshot.action_decisions[action_index];
        result["observations"][action_name] = snapshot.observations[action_index];
        for (size_t reason_index = 0; reason_index < server_kv_action_reason_count; ++reason_index) {
            const auto reason = static_cast<server_kv_action_reason>(reason_index);
            result["action_reasons"][action_name][server_kv_action_reason_name(reason)] =
                    snapshot.action_reason_decisions[action_index][reason_index];
        }
    }
    for (size_t reason_index = 0; reason_index < server_kv_action_reason_count; ++reason_index) {
        const auto reason = static_cast<server_kv_action_reason>(reason_index);
        result["reasons"][server_kv_action_reason_name(reason)] = snapshot.reason_decisions[reason_index];
    }
    return result;
}
...
json server_task_result_metrics::to_json() {
    return json {
        ...
        { "n_slot_cache_selections_total",   n_slot_cache_selections_total },
        { "n_slot_lru_selections_total",     n_slot_lru_selections_total },
        { "n_slot_reused_tokens_total",      n_slot_reused_tokens_total },
        { "n_slot_evicted_tokens_total",     n_slot_evicted_tokens_total },
        { "n_scheduler_iterations_total",     n_scheduler_iterations_total },
        { "n_decode_tokens_scheduled_total",  n_decode_tokens_scheduled_total },
        { "n_prefill_tokens_scheduled_total", n_prefill_tokens_scheduled_total },
        { "n_prefill_chunks_scheduled_total", n_prefill_chunks_scheduled_total },
        { "batch_tokens", batch_tokens },
        { "batch_sequences", batch_sequences },
        { "prefill_starvation_ms", prefill_starvation_ms },
        { "n_kv_admission_pressure_total", n_kv_admission_pressure_total },
        { "n_kv_proactive_evictions_total", n_kv_proactive_evictions_total },
        { "n_kv_proactive_reclaimed_tokens_total", n_kv_proactive_reclaimed_tokens_total },
        { "n_kv_admission_failures_total", n_kv_admission_failures_total },
        { "benefit_cpu", {
            { "upstream_observations", benefit_cpu.upstream_observations },
            { "cacheflow_observations", benefit_cpu.cacheflow_observations },
            { "upstream_decisions", benefit_cpu.upstream_decisions },
            { "cacheflow_decisions", benefit_cpu.cacheflow_decisions },
            { "exploration_decisions", benefit_cpu.exploration_decisions },
            { "cold_start_decisions", benefit_cpu.cold_start_decisions },
            { "positive_lower_bound_decisions", benefit_cpu.positive_lower_bound_decisions },
            { "insufficient_evidence_decisions", benefit_cpu.insufficient_evidence_decisions },
            { "safety_fallbacks", benefit_cpu.safety_fallbacks },
            { "drift_events", benefit_cpu.drift_events },
            { "cooldown_remaining", benefit_cpu.cooldown_remaining },
            { "predicted_benefit_ms", benefit_cpu.last_predicted_benefit_ms },
            { "uncertainty_ms", benefit_cpu.last_uncertainty_ms }
        } },
        { "benefit_cuda", {
            { "upstream_observations", benefit_cuda.upstream_observations },
            { "cacheflow_observations", benefit_cuda.cacheflow_observations },
            { "upstream_decisions", benefit_cuda.upstream_decisions },
            { "cacheflow_decisions", benefit_cuda.cacheflow_decisions },
            { "exploration_decisions", benefit_cuda.exploration_decisions },
            { "cold_start_decisions", benefit_cuda.cold_start_decisions },
            { "positive_lower_bound_decisions", benefit_cuda.positive_lower_bound_decisions },
            { "insufficient_evidence_decisions", benefit_cuda.insufficient_evidence_decisions },
            { "safety_fallbacks", benefit_cuda.safety_fallbacks },
            { "drift_events", benefit_cuda.drift_events },
            { "cooldown_remaining", benefit_cuda.cooldown_remaining },
            { "predicted_benefit_ms", benefit_cuda.last_predicted_benefit_ms },
            { "uncertainty_ms", benefit_cuda.last_uncertainty_ms }
        } },
        { "kv_action_policy", server_kv_action_policy_to_json(kv_action_policy) },

        { "kv_cache_tokens",                 kv_cache_tokens },
        { "kv_cache_capacity_tokens",        kv_cache_capacity_tokens },

        { "memory_model_bytes",              memory_model_bytes },
        { "memory_context_bytes",            memory_context_bytes },
        { "memory_compute_bytes",            memory_compute_bytes },
