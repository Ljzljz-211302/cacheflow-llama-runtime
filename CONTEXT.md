...
## Matched Process Block

在相同冻结 workload 集、模型、设备与服务配置下，按预注册随机顺序分别启动 Direct 和 Paged 独立进程所得的一组对照观测。统计时以整个进程块为重采样单位，保留同一进程内 workload 之间的相关性。它用于降低进程启动与测量时段差异，但两臂不共享同一进程或热状态，因此不满足 Trial Pair 的因果配对条件。

_Avoid_: Trial Pair；把独立进程的匹配观测解释成共享缓存状态下的单变量干预。
