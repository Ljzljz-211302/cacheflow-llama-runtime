...
struct common_params {
    ...
    std::string scheduler_policy = "upstream";
    std::string prefill_policy = "greedy";
    std::string benefit_policy = "upstream";
    int32_t benefit_min_observations = 6;
    int32_t benefit_exploration_interval = 8;
    float benefit_confidence_beta = 1.0f;
    float benefit_safety_margin_ms = 0.25f;
    float benefit_drift_ratio = 2.5f;
    int32_t benefit_drift_consecutive = 3;
    int32_t benefit_cooldown_decisions = 16;
    std::string benefit_checkpoint_path;
    std::string benefit_checkpoint_key;
    int32_t benefit_checkpoint_interval = 128;
    std::string spec_policy = "fixed";
    float slot_cache_eviction_penalty = 0.0f;
    int32_t prefill_chunk_size = 0;
    bool    prefill_chunk_adaptive = false;
    int32_t prefill_chunk_min = 16;
    int32_t prefill_chunk_max = 512;
    float   prefill_target_iteration_ms = 25.0f;
    bool    speculative_adaptive = false;
    float   speculative_disable_acceptance = 0.20f;
    float   speculative_kv_pressure = 0.85f;
    int32_t speculative_cooldown = 8;
    bool    kv_block_runtime = false;
    int32_t kv_block_size = 16;
    std::string kv_swap_path;
    int32_t kv_swap_budget_mib = 0;
    std::string kv_swap_fault = "none";
    std::string kv_action_policy = "learned-shadow";
    bool kv_paged_decode = false;
    enum class kv_action_override : uint8_t {
        none,
        direct,
        remap,
        paged,
    ...
    kv_action_override kv_action_override_mode = kv_action_override::none;
    int32_t kv_action_min_observations = 8;
    float kv_action_confidence_beta = 1.0f;
    float kv_action_switch_margin_ms = 0.25f;
    std::string engine_trace_path;
