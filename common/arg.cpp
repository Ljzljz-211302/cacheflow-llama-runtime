...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            if (value == "upstream") {
                ...
                params.kv_action_policy = "learned-shadow";
            } else {
    ...
    add_opt(common_arg(
        ...
        "residual/prediction ratio required to count a mature-model drift sample",
        [](common_params & params, const std::string & value) {
            params.benefit_drift_ratio = std::stof(value);
            if (params.benefit_drift_ratio <= 0.0f) throw std::invalid_argument("benefit drift ratio must be positive");
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--benefit-drift-consecutive"}, "N",
        "consecutive severe residuals required before model reset",
        [](common_params & params, int value) {
            if (value <= 0) throw std::invalid_argument("benefit drift consecutive count must be positive");
            params.benefit_drift_consecutive = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--benefit-cooldown-decisions"}, "N",
        "upstream fallback decisions after a drift reset",
        [](common_params & params, int value) {
            if (value < 0) throw std::invalid_argument("benefit cooldown decisions must be non-negative");
            params.benefit_cooldown_decisions = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--benefit-checkpoint"}, "PATH",
        "persist learned benefit-policy state asynchronously (use one path per server replica)",
        [](common_params & params, const std::string & value) {
            if (value.empty()) throw std::invalid_argument("benefit checkpoint path must not be empty");
            params.benefit_checkpoint_path = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--benefit-checkpoint-key"}, "KEY",
        "explicit model/deployment identity for checkpoint compatibility (default: derived from loaded model)",
        [](common_params & params, const std::string & value) {
            if (value.empty()) throw std::invalid_argument("benefit checkpoint key must not be empty");
            params.benefit_checkpoint_key = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--benefit-checkpoint-interval"}, "N",
        "observations between asynchronous benefit checkpoints (default: 128)",
        [](common_params & params, int value) {
            if (value <= 0) throw std::invalid_argument("benefit checkpoint interval must be positive");
            params.benefit_checkpoint_interval = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        {"--kv-action-policy"}, "fixed|analytical|learned-shadow|learned",
        "select unified Direct/Remap/Paged/Swap/Recompute policy (default: learned-shadow)",
        [](common_params & params, const std::string & value) {
            if (value != "fixed" && value != "analytical" &&
                    value != "learned-shadow" && value != "learned") {
                throw std::invalid_argument(
                        "KV action policy must be fixed, analytical, learned-shadow, or learned");
            }
            params.kv_action_policy = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--kv-paged-decode"},
        "enable the evidence-gated production paged-decode capability",
        [](common_params & params) {
            params.kv_paged_decode = true;
            params.kv_block_runtime = true;
            params.kv_block_size = 16;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--kv-action-override"}, "none|direct|remap|paged|recompute",
        "controlled action selection for correctness and application-journey tests",
        [](common_params & params, const std::string & value) {
            using override = common_params::kv_action_override;
            if (value == "none") {
                params.kv_action_override_mode = override::none;
            } else if (value == "direct") {
                params.kv_action_override_mode = override::direct;
            } else if (value == "remap") {
                params.kv_action_override_mode = override::remap;
            } else if (value == "paged") {
                params.kv_action_override_mode = override::paged;
            } else if (value == "recompute") {
                params.kv_action_override_mode = override::recompute;
            } else {
                throw std::invalid_argument("KV action override is invalid");
            }
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--kv-action-min-observations"}, "N",
        "completed observations required per action before learned comparison",
        [](common_params & params, int value) {
            if (value <= 0) throw std::invalid_argument("KV action minimum observations must be positive");
            params.kv_action_min_observations = value;
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--kv-action-confidence-beta"}, "BETA",
        "confidence radius multiplier for learned KV action selection",
        [](common_params & params, const std::string & value) {
            params.kv_action_confidence_beta = std::stof(value);
            if (params.kv_action_confidence_beta < 0.0f) {
                throw std::invalid_argument("KV action confidence beta must be non-negative");
            }
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--kv-action-switch-margin-ms"}, "MS",
        "minimum pessimistic cost advantage required to leave the fixed H0 action",
        [](common_params & params, const std::string & value) {
            params.kv_action_switch_margin_ms = std::stof(value);
            if (params.kv_action_switch_margin_ms < 0.0f) {
                throw std::invalid_argument("KV action switch margin must be non-negative");
            }
        }
    ).set_examples({LLAMA_EXAMPLE_SERVER}));
    add_opt(common_arg(
