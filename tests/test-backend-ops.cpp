...
// Restricted production Paged path: two logical pages are deliberately mapped
// out of physical order with a gap. CPU and CUDA both consume the public Paged
// operator contract, so a CUDA implementation that ignores block_table cannot
// match the CPU oracle.
struct test_flash_attn_ext_paged : public test_case {
    const std::vector<int> contexts;
    const bool stream_layout;

    explicit test_flash_attn_ext_paged(int context) : contexts({ context }), stream_layout(false) {}
    explicit test_flash_attn_ext_paged(std::vector<int> contexts, bool stream_layout = false) :
        contexts(std::move(contexts)), stream_layout(stream_layout) {}

    std::string vars() override {
        return "hsk=64,nh=14,nkv=2,kv=64,batch=" + std::to_string(contexts.size()) +
                ",context_min=" + std::to_string(*std::min_element(contexts.begin(), contexts.end())) +
                ",layout=" + std::string(stream_layout ? "stream" : "unified") +
                ",context_max=" + std::to_string(*std::max_element(contexts.begin(), contexts.end())) +
                ",page=16";
    }

    double max_nmse_err() override {
        // The generic CPU Flash Attention reference accumulates in a
        // different order from both CUDA K1 and tiled K2. Its NMSE grows at
        // 128 pages even though K1 and K2 agree, so keep a declared
        // length-dependent bound instead of silently dropping long cases.
        if (*std::max_element(contexts.begin(), contexts.end()) > 1025) {
            return 6e-3;
        }
        return 6e-4;
    }

    int physical_tokens() const {
        int count = 0;
        for (int context : contexts) {
            if (stream_layout) count = std::max(count, ((context + 15) / 16) * 16);
            else count += ((context + 15) / 16) * 16;
        }
        return std::max(64, count);
    }

    int sequence_physical_offset(int sequence) const {
        if (stream_layout) return 0;
        int offset = 0;
        for (int index = 0; index < sequence; ++index) {
            offset += ((contexts[index] + 15) / 16) * 16;
        }
        return offset;
    }

    int physical_cell(int sequence, int token) const {
        const int pages = (contexts[sequence] + 15) / 16;
        return sequence_physical_offset(sequence) +
                (pages - token / 16 - 1) * 16 + token % 16;
    }

    ggml_tensor * build_graph(ggml_context * ctx) override {
        const int batch = static_cast<int>(contexts.size());
        const int max_pages = (*std::max_element(contexts.begin(), contexts.end()) + 15) / 16;
        ggml_tensor * q = ggml_new_tensor_4d(ctx, GGML_TYPE_F32, 64,
                stream_layout ? 1 : batch, 14, stream_layout ? batch : 1);
        ggml_tensor * k = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, 64, physical_tokens(), 2,
                stream_layout ? batch : 1);
        ggml_tensor * v = ggml_new_tensor_4d(ctx, GGML_TYPE_F16, 64, physical_tokens(), 2,
                stream_layout ? batch : 1);
        ggml_tensor * block_table = ggml_new_tensor_2d(ctx, GGML_TYPE_I32, max_pages, batch);
        ggml_tensor * context_lengths = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, batch);
        ggml_set_name(q, "q");
        ggml_set_name(k, "k");
        ggml_set_name(v, "v");
        ggml_set_name(block_table, "block_table");
        ggml_set_name(context_lengths, "context_lengths");
        ggml_tensor * out = ggml_flash_attn_ext(ctx, q, k, v, nullptr, 1.0f/sqrtf(64), 0.0f, 0.0f);
        ggml_flash_attn_ext_set_paged(out, block_table, context_lengths, 16);
        ggml_set_name(out, "out");
        return out;
    }
    ...
    void initialize_tensors(ggml_context * ctx) override {
        for (ggml_tensor * tensor = ggml_get_first_tensor(ctx); tensor != nullptr;
            ...
            if (strcmp(tensor->name, "block_table") == 0) {
                const int max_pages = static_cast<int>(tensor->ne[0]);
                std::vector<int32_t> values(ggml_nelements(tensor), 0);
                for (int sequence = 0; sequence < static_cast<int>(contexts.size()); ++sequence) {
                    const int pages = (contexts[sequence] + 15) / 16;
                    for (int logical_page = 0; logical_page < pages; ++logical_page) {
                        values[sequence * max_pages + logical_page] = sequence_physical_offset(sequence) +
                                (pages - logical_page - 1) * 16;
                    }
                }
                ggml_backend_tensor_set(tensor, values.data(), 0,
                        values.size() * sizeof(values[0]));
            } else if (strcmp(tensor->name, "context_lengths") == 0) {
                std::vector<int32_t> values(contexts.begin(), contexts.end());
                ggml_backend_tensor_set(tensor, values.data(), 0, values.size() * sizeof(values[0]));
            } else if (strcmp(tensor->name, "q") == 0) {
                std::vector<float> values(ggml_nelements(tensor));
                for (int head = 0; head < 14; ++head) {
                    for (int sequence = 0; sequence < static_cast<int>(contexts.size()); ++sequence) {
                        for (int dim = 0; dim < 64; ++dim) {
                            const size_t index = stream_layout ?
                                    dim + 64 * head + 64 * 14 * sequence :
                                    dim + 64 * (sequence + contexts.size() * head);
                            values[index] = 0.013f * (head + 1) + 0.002f * sequence -
                                    0.0007f * dim + 0.00001f * (int) ((index * 17 + 11) % 29);
                        }
                    }
                }
                ggml_backend_tensor_set(tensor, values.data(), 0, values.size() * sizeof(values[0]));
            } else if (strcmp(tensor->name, "k") == 0) {
                std::vector<ggml_fp16_t> values(ggml_nelements(tensor),
                                                ggml_fp32_to_fp16(-50.0f));
                for (int head = 0; head < 2; ++head) {
                    for (int sequence = 0; sequence < static_cast<int>(contexts.size()); ++sequence) {
                        for (int token = 0; token < contexts[sequence]; ++token) {
                            const int physical = physical_cell(sequence, token);
                            for (int dim = 0; dim < 64; ++dim) {
                                const size_t index = dim + 64 * physical +
                                        64 * (size_t) physical_tokens() * head +
                                        (stream_layout ? 64 * (size_t) physical_tokens() * 2 * sequence : 0);
                                values[index] = ggml_fp32_to_fp16(
                                        0.0002f * (token % 251) + 0.0001f * head +
                                        0.0003f * sequence + 0.00001f * dim +
                                        0.000003f * ((token * 13 + dim) % 7));
                            }
                        }
                    }
                }
                ggml_backend_tensor_set(tensor, values.data(), 0, values.size() * sizeof(values[0]));
            } else if (strcmp(tensor->name, "v") == 0) {
                std::vector<ggml_fp16_t> values(ggml_nelements(tensor),
                                                ggml_fp32_to_fp16(-1000.0f));
                for (int head = 0; head < 2; ++head) {
                    for (int sequence = 0; sequence < static_cast<int>(contexts.size()); ++sequence) {
                        for (int token = 0; token < contexts[sequence]; ++token) {
                            const int physical = physical_cell(sequence, token);
                            for (int dim = 0; dim < 64; ++dim) {
                                const size_t index = dim + 64 * physical +
                                        ...
                                        (stream_layout ? 64 * (size_t) physical_tokens() * 2 * sequence : 0);
                                values[index] = ggml_fp32_to_fp16(
                                        0.01f * static_cast<float>(token % 257) + 2.0f * head +
                                        0.25f * sequence + 0.001f * dim +
                                        0.0001f * ((token * 5 + dim * 3) % 11));
                            }
                        }
                    }
                }
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    // Page and dispatch boundaries for the production Paged operator:
    // One-page boundaries plus ragged/full boundaries across 2, 4, 8, 16,
    // and 128 pages. Reversed physical pages make every multi-page case prove
    // that the production CUDA op follows the block table.
    for (int context : {1, 15, 16, 17, 31, 32, 33, 63, 64, 65,
                        127, 128, 129, 255, 256, 257, 511, 512, 513,
                        1023, 1024, 1025, 2047, 2048}) {
        test_cases.emplace_back(new test_flash_attn_ext_paged(context));
    }
    test_cases.emplace_back(new test_flash_attn_ext_paged(std::vector<int>{17, 33}));
    test_cases.emplace_back(new test_flash_attn_ext_paged(std::vector<int>{512, 513}));
    test_cases.emplace_back(new test_flash_attn_ext_paged(std::vector<int>{15, 64, 129, 257}));
    test_cases.emplace_back(new test_flash_attn_ext_paged(
            std::vector<int>{1, 16, 31, 65, 128, 513, 1025, 2048}));
    test_cases.emplace_back(new test_flash_attn_ext_paged(std::vector<int>{17, 33}, true));
    test_cases.emplace_back(new test_flash_attn_ext_paged(std::vector<int>{512, 513}, true));
    test_cases.emplace_back(new test_flash_attn_ext_paged(std::vector<int>{15, 64, 129, 257}, true));
    test_cases.emplace_back(new test_flash_attn_ext_paged(
            std::vector<int>{1, 16, 31, 65, 128, 513, 1025, 2048}, true));
