...
ggml_tensor * llama_kv_cache_context::build_input_paged_decode_table(
        ggml_context * ctx, const llama_ubatch & ubatch) const {
    auto * table = ggml_new_tensor_2d(ctx, GGML_TYPE_I32, (n_kv + 15) / 16, ubatch.n_tokens);
    ggml_set_input(table);
    ggml_set_name(table, "paged_decode_block_table");
    return table;
}

ggml_tensor * llama_kv_cache_context::build_input_paged_decode_length(
        ggml_context * ctx, const llama_ubatch & ubatch) const {
    auto * length = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, ubatch.n_tokens);
    ggml_set_input(length);
    ggml_set_name(length, "paged_decode_context_length");
    return length;
}
...
void llama_kv_cache_context::set_input_paged_decode(
        ggml_tensor * table,
        ggml_tensor * length,
        const llama_ubatch * ubatch) const {
    GGML_ASSERT(table && length);
    GGML_ASSERT(ggml_backend_buffer_is_host(table->buffer));
    GGML_ASSERT(ggml_backend_buffer_is_host(length->buffer));
    const auto & layout = prepared_paged_decode_layout;
    GGML_ASSERT(layout.context_lengths.size() == ubatch->n_tokens);
    GGML_ASSERT(table->ne[1] == ubatch->n_tokens);
    GGML_ASSERT(length->ne[0] == ubatch->n_tokens);
    GGML_ASSERT(!layout.block_table.empty());
    GGML_ASSERT(layout.max_blocks <= static_cast<uint32_t>(table->ne[0]));
    auto * table_data = static_cast<int32_t *>(table->data);
    std::fill(table_data, table_data + ggml_nelements(table), 0);
    for (size_t sequence = 0; sequence < layout.context_lengths.size(); ++sequence) {
        GGML_ASSERT(layout.context_lengths[sequence] == static_cast<uint32_t>(ubatch->pos[sequence] + 1));
        for (uint32_t block = 0; block < layout.max_blocks; ++block) {
            table_data[sequence * table->ne[0] + block] = static_cast<int32_t>(
                    layout.block_table[sequence * layout.max_blocks + block]);
        }
    }
    auto * length_data = static_cast<int32_t *>(length->data);
    for (size_t sequence = 0; sequence < layout.context_lengths.size(); ++sequence) {
        length_data[sequence] = static_cast<int32_t>(layout.context_lengths[sequence]);
    }
}

llama_paged_decode_layout_status llama_kv_cache_context::build_paged_decode_layout(
        const llama_ubatch & ubatch,
        llama_paged_decode_layout & output) const {
    const auto status = kv->build_paged_decode_layout(ubatch, sinfos[i_cur], output);
    prepared_paged_decode_layout = status == llama_paged_decode_layout_status::success ?
            output : llama_paged_decode_layout{};
    return status;
}

llama_paged_decode_layout_status llama_kv_cache::build_paged_decode_layout(
        const llama_ubatch & ubatch,
        const slot_info & sinfo,
        llama_paged_decode_layout & output) const {
    output = {};
    if (cacheflow_block_tokens_ != 16 || type_k() != GGML_TYPE_F16 ||
            type_v() != GGML_TYPE_F16 || v_trans || ubatch.n_tokens == 0 ||
            ubatch.n_tokens != ubatch.n_seqs_unq) {
        return llama_paged_decode_layout_status::unsupported_request;
    }

    // prepare() selected the physical destination but has not committed its
    // metadata yet. Overlay it so capability checking remains pre-mutation.
    if (sinfo.n_stream() == 0 ||
            (sinfo.n_stream() == 1 && sinfo.idxs[0].size() != ubatch.n_tokens)) {
        return llama_paged_decode_layout_status::unsupported_request;
    }

    std::vector<std::vector<llama_paged_decode_cell>> sequences(ubatch.n_tokens);
    std::vector<bool> seen(seq_to_stream.size(), false);
    for (uint32_t token_index = 0; token_index < ubatch.n_tokens; ++token_index) {
        if (ubatch.n_seq_id[token_index] != 1) {
            return llama_paged_decode_layout_status::unsupported_request;
        }
        const llama_seq_id seq_id = ubatch.seq_id[token_index][0];
        if (seq_id < 0 || static_cast<size_t>(seq_id) >= seq_to_stream.size() || seen[seq_id]) {
            return llama_paged_decode_layout_status::unsupported_request;
        }
        seen[seq_id] = true;
        const uint32_t stream = seq_to_stream[seq_id];
        const auto & cells = v_cells[stream];
        auto & sequence_cells = sequences[token_index];
        sequence_cells.reserve(cells.get_used() + 1);
        for (uint32_t cell = 0; cell < cells.size(); ++cell) {
            if (!cells.is_empty(cell) && cells.seq_has(cell, seq_id)) {
                ...
                if (position >= 0 && position <= ubatch.pos[token_index]) {
                    sequence_cells.push_back({ static_cast<uint32_t>(position), cell });
                }
            }
        }

        uint32_t pending_cell = 0;
        if (sinfo.n_stream() == 1) {
            pending_cell = sinfo.idxs[0][token_index];
        } else {
            const auto stream_it = std::find(sinfo.strm.begin(), sinfo.strm.end(), stream);
            if (stream_it == sinfo.strm.end()) {
                return llama_paged_decode_layout_status::unsupported_request;
            }
            const size_t pending_stream = static_cast<size_t>(stream_it - sinfo.strm.begin());
            if (sinfo.idxs[pending_stream].size() != 1) {
                return llama_paged_decode_layout_status::unsupported_request;
            }
            pending_cell = sinfo.idxs[pending_stream][0];
        }

        // A cached tail at the same logical position is intentionally
        // recomputed. The pending destination replaces stale metadata.
        sequence_cells.erase(std::remove_if(sequence_cells.begin(), sequence_cells.end(),
                [&ubatch, token_index](const llama_paged_decode_cell & item) {
                    return item.logical_position == static_cast<uint32_t>(ubatch.pos[token_index]);
                }), sequence_cells.end());
        sequence_cells.push_back({ static_cast<uint32_t>(ubatch.pos[token_index]), pending_cell });
    }
    return llama_paged_decode_build_batch_layout(sequences, cacheflow_block_tokens_, output);
}
