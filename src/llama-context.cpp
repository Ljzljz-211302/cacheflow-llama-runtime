...
void llama_context::set_kv_execution_action(enum llama_kv_execution_action action) {
    kv_execution_status.requested = action;
}

llama_kv_execution_status llama_context::get_kv_execution_status() const {
    return kv_execution_status;
}
...
int llama_context::decode(const llama_batch & batch_inp) {
    ...
    do {
        ...
        paged_decode_effective = false;
        kv_execution_status.effective = kv_execution_status.requested;
        kv_execution_status.fallback_reason = LLAMA_KV_EXECUTION_FALLBACK_NONE;
        if (kv_execution_status.requested == LLAMA_KV_EXECUTION_ACTION_PAGED) {
            llama_paged_decode_layout layout;
            const auto * kv_mctx = dynamic_cast<llama_kv_cache_context *>(mctx.get());
            const auto layout_status = kv_mctx ? kv_mctx->build_paged_decode_layout(ubatch, layout) :
                    llama_paged_decode_layout_status::unsupported_request;
            const bool model_supported = model.arch == LLM_ARCH_QWEN2 &&
                    model.hparams.n_head() == 14 && model.hparams.n_head_kv() == 2 &&
                    model.hparams.n_embd_head_k() == 64 && model.hparams.n_layer() == 24 &&
                    model.hparams.f_max_alibi_bias == 0.0f && !model.hparams.attn_soft_cap &&
                    model.n_gpu_layers() > model.hparams.n_layer();
            const bool contexts_supported = layout_status == llama_paged_decode_layout_status::success &&
                    std::all_of(layout.context_lengths.begin(), layout.context_lengths.end(),
                            [](uint32_t length) { return length <= 2048; });
            const bool paged_supported = contexts_supported && model_supported && cparams.flash_attn &&
                    cparams.causal_attn;
            static const bool contiguous_fastpath_enabled = [] {
                const char * value = std::getenv("LLAMA_CACHEFLOW_PAGED_CONTIGUOUS_FASTPATH");
                return !value || std::strcmp(value, "0") != 0;
            }();
            const bool contiguous_fastpath = paged_supported && contiguous_fastpath_enabled &&
                    layout.physically_contiguous;
            paged_decode_effective = paged_supported && !contiguous_fastpath;
            if (contiguous_fastpath) {
                kv_execution_status.paged_decode_contiguous_fastpath_calls++;
                kv_execution_status.paged_decode_contiguous_fastpath_sequences +=
                        layout.context_lengths.size();
            } else if (paged_decode_effective) {
                kv_execution_status.paged_decode_calls++;
                kv_execution_status.paged_decode_sequences += layout.context_lengths.size();
                kv_execution_status.paged_decode_max_batch = std::max<uint32_t>(
                        kv_execution_status.paged_decode_max_batch,
                        static_cast<uint32_t>(layout.context_lengths.size()));
            } else if (!paged_supported) {
                kv_execution_status.effective = LLAMA_KV_EXECUTION_ACTION_DIRECT;
                switch (layout_status) {
                    case llama_paged_decode_layout_status::success:
                        kv_execution_status.fallback_reason = LLAMA_KV_EXECUTION_FALLBACK_UNSUPPORTED_MODEL;
                        break;
                    case llama_paged_decode_layout_status::invalid_page_size:
                        kv_execution_status.fallback_reason = LLAMA_KV_EXECUTION_FALLBACK_INVALID_PAGE_SIZE;
                        break;
                    case llama_paged_decode_layout_status::empty_sequence:
                        kv_execution_status.fallback_reason = LLAMA_KV_EXECUTION_FALLBACK_EMPTY_SEQUENCE;
                        break;
                    case llama_paged_decode_layout_status::missing_position:
                        kv_execution_status.fallback_reason = LLAMA_KV_EXECUTION_FALLBACK_MISSING_POSITION;
                        break;
                    case llama_paged_decode_layout_status::duplicate_position:
                        kv_execution_status.fallback_reason = LLAMA_KV_EXECUTION_FALLBACK_DUPLICATE_POSITION;
                        break;
                    case llama_paged_decode_layout_status::non_contiguous_block:
                        kv_execution_status.fallback_reason = LLAMA_KV_EXECUTION_FALLBACK_NON_CONTIGUOUS_BLOCK;
                        break;
                    case llama_paged_decode_layout_status::unsupported_request:
                        kv_execution_status.fallback_reason = LLAMA_KV_EXECUTION_FALLBACK_UNSUPPORTED_REQUEST;
                        break;
                }
                kv_execution_status.paged_decode_fallbacks++;
            }
        }
...
llm_graph_params llama_context::graph_params(
    ...
    return {
        ...
        /*.ubatch      =*/ ubatch,
        /*.gtype       =*/ gtype,
        /*.paged_decode=*/ paged_decode_effective,
        /*.sched       =*/ sched.get(),
...
void llama_set_kv_execution_action(
        llama_context * ctx,
        enum llama_kv_execution_action action) {
    ctx->set_kv_execution_action(action);
}

struct llama_kv_execution_status llama_get_kv_execution_status(const llama_context * ctx) {
    return ctx->get_kv_execution_status();
}
...
bool llama_memory_seq_can_swap_cuda(llama_memory_t mem, llama_seq_id seq_id) {
    return mem && mem->seq_can_swap_cuda(seq_id);
}

bool llama_memory_seq_swap_out_cuda(llama_memory_t mem, llama_seq_id seq_id) {
    return mem && mem->seq_swap_out_cuda(seq_id);
}

bool llama_memory_seq_swap_in_cuda(llama_memory_t mem, llama_seq_id seq_id) {
    return mem && mem->seq_swap_in_cuda(seq_id);
}

void llama_memory_seq_swap_erase_cuda(llama_memory_t mem, llama_seq_id seq_id) {
    if (mem) mem->seq_swap_erase_cuda(seq_id);
}

bool llama_memory_cacheflow_set_block_size(llama_memory_t mem, uint32_t block_tokens) {
    return mem && mem->cacheflow_set_block_size(block_tokens);
}

bool llama_memory_cacheflow_cuda_stats(
        llama_memory_t mem, llama_cacheflow_cuda_stats * stats) {
    return mem && stats && mem->cacheflow_cuda_stats(*stats);
}
