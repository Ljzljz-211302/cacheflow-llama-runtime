...
void llm_graph_input_attn_kv::set_input(const llama_ubatch * ubatch) {
    ...
    if (self_paged_decode_table) {
        mctx->set_input_paged_decode(self_paged_decode_table, self_paged_decode_length, ubatch);
    }
}
...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    paged_decode     (params.paged_decode),
...
ggml_tensor * llm_graph_context::build_attn_mha(
         ...
         ggml_tensor * v_mla,
               float   kq_scale,
                 int   il,
       ggml_tensor * paged_decode_table,
       ggml_tensor * paged_decode_length) const {
    ...
    if (use_flash_attn) {
        ...
        if (paged_decode_table) {
            GGML_ASSERT(sinks == nullptr);
            ggml_flash_attn_ext_set_paged(cur, paged_decode_table, paged_decode_length, 16);
        }
...
static std::unique_ptr<llm_graph_input_attn_kv> build_attn_inp_kv_impl(
...

    {
        ...
        inp->self_kq_mask_cnv = inp->self_kq_mask;
        if (paged_decode) {
            inp->self_paged_decode_table  = mctx_cur->build_input_paged_decode_table(ctx0, ubatch);
            inp->self_paged_decode_length = mctx_cur->build_input_paged_decode_length(ctx0, ubatch);
        }
    }
...
llm_graph_input_attn_kv * llm_graph_context::build_attn_inp_kv() const {
    ...
    auto inp = build_attn_inp_kv_impl(ctx0, ubatch, hparams, cparams, mctx_cur, paged_decode);
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    ggml_tensor * cur = build_attn_mha(q, k, v, kq_b, kq_mask, sinks, v_mla, kq_scale, il,
            inp->get_paged_decode_table(), inp->get_paged_decode_length());
    cb(cur, "kqv_out", il);
...
llm_graph_input_mem_hybrid * llm_graph_context::build_inp_mem_hybrid() const {
    ...
    auto inp_attn = build_attn_inp_kv_impl(ctx0, ubatch, hparams, cparams, mctx_cur->get_attn(), false);
